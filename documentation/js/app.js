...
app.controller('MainCtrl', function ($scope, $http, $location, docobjects) {
	...
	//all namespaces sorted by alphabit order and length order
	$scope.namespaces = [];
	...
	$scope.expandSection = function (section, isExpand) {
		if (!section) return;
	...
	function getData(path, objname, type) {
		...
		if (objname == '') {
			objname = 'index';
			type = 'modules';
			$http.get(path + '/' + objname + ".json").success(function (data, status) {
