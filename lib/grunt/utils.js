...
module.exports = {
	...
	getDemos: function (demosSourceDirectory) {
		var demos = [];
		grunt.file.expand({ filter: 'isDirectory' }, path.join(demosSourceDirectory, "*")).forEach(function (dir) {
			
			if (grunt.file.exists(path.join(dir, "demo.xml"))) {
				demos.push("demo:" + path.basename(dir));
			}
		});
