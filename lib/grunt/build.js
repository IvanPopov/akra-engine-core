...
var DEMOS_DIRECTORY = "$(BuiltDir)/demos";
 ...
 */\n";
		...
		config.find("//PropertyGroup/Resource").forEach(function (resource) {
			...
			if (meta.outputFile) {
				if (exists(meta.outputFile)) {
					...
					debug("file " + meta.outputFile + ' ' + fs.lstatSync(meta.outputFile).size + ' bytes');
				}
				else {
	...
	function getPreview(config) {
		var preview = config.get('Preview') ? config.get('Preview').getAttribute("Src") : null;
		var youtube = null;

		var outDir = prepareSystemVariables(DEMOS_DIRECTORY);
		var name = config.getAttribute("Name");
		...
		compile(config, function (ok, modulesInfo) {
			...
			var html = fn({
				demo: {
					...
					youtube: getPreview(config).youtube
				}
			});
		...
		if (!demo) {
			...
			grunt.file.expand({ filter: 'isDirectory' }, path.join(cfg('DemosSourceDir'), "*")).forEach(function (dir) {
				...
				if (exists(configFile)) {
					...
					var previewObject = getPreview(config);

					var preview = previewObject.preview;
					var youtube = previewObject.youtube;
			...
			grunt.task.registerTask('showcase-page', '...', function () {
			...
			if (!grunt.option("list") && !grunt.option("showcase-page")) {
				if (grunt.option("parallel")) {
					grunt.task.run(["parallel:demos", "showcase-page"]);
				}
				else {
					grunt.task.run(all.concat(['showcase-page']));
				}
			}

			if (grunt.option("showcase-page")) {
				grunt.task.run(['showcase-page']);
			}
