...
module akra.math {
	...
	export class Mat4 implements IMat4 {
				...
				switch (n) {
					case 1:
						if (arguments[0] instanceof Mat3) {
							this.set(arguments[0], Vec3.temp(0.));
						}
						else {
							this.set(arguments[0]);
						}
		...
		set(f: float): IMat4;
		set(v4: IVec4): IMat4;
		set(m3: IMat3, v3?: IVec3): IMat4;
		set(m4: IMat4): IMat4;
		set(p: float[]): IMat4;
		set(f1: float, f2: float,
			f3: float, f4: float): IMat4;
		set(v1: IVec4, v2: IVec4,
			v3: IVec4, v4: IVec4): IMat4;
		set(p1: float[], p2: float[],
			p3: float[], p4: float[]): IMat4;
		...
		isEqual(m4fMat: IMat4, fEps: float = 0.): boolean {
			var pData1: Float32Array = this.data;
			var pData2: Float32Array = m4fMat.data;
		...
		scaleRight(scale: any, m4fDestination?: IMat4): IMat4 {
			var pData: Float32Array = this.data;
		...
		scaleLeft(scale: any, m4fDestination?: IMat4): IMat4 {
			var pData: Float32Array = this.data;
		...
		/**  */ static perspective(fFovy: float, fAspect: float, fNear: float, fFar: float, m4fDestination?: IMat4): IMat4 {
		...
		/**  */ static orthogonalProjection(fWidth: float, fHeight: float, fNear: float, fFar: float, m4fDestination?: IMat4): IMat4 {
		...
		static temp(
			...
			vColumn2: IVec4,
			vColumn3: IVec4,
			vColumn4: IVec4): IMat4;
		static temp(
			pColumn1: float[],
			pColumn2: float[],
			pColumn3: float[],
			pColumn4: float[]): IMat4;
		static temp(
			f1: float, f2: float, f3: float, f4: float,
			f5: float, f6: float, f7: float, f8: float,
			f9: float, f10: float, f11: float, f12: float,
	...
	pBuffer = gen.array<IMat4>(256, Mat4);
	iElement = 0;
