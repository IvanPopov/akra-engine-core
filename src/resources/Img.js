...
Img.prototype.getData = function (iMipLevel, eCubeFlag, iVolumeLevel) {
    ...
    if (TEST_BIT(this._iFlags, a.Img.CubeMap)) {
        debug_assert(TEST_BIT(this._iCubeFlags, eCubeFlag),
                     "Запрашивается часть кубической текстуры которой нет, которого нет");
    }
    else {
    ...
    for (var i = 0; i < this._pData.length; i++) {
        for (var k = 0; k < this._pData[i].length; k++) {
            for (var l = 0; l < this._pData[i][k].length; l++) {
                this._pData[i][k][l] = new ArrayBuffer((Math.ceil(iWidth / this.getDivSize()) *
                                                        Math.ceil(iHeight / this.getDivSize()) *
                                                        this.getBlockBytes()) >>> (l * 2));
            }
        }
    }
    ;
    ...
    if ((sFileName.nodeName) && sFileName.nodeName.toLowerCase() == "canvas") {
        ...
        a.fopen(sFileName, 'rb').onread = function (pData) {
            ...
            if (dwMagic4 == DDS_MAGIC) //DDS
            {
                ...
                if (TEST_BIT(me._iFlags, a.Img.MipMaps) &&
                    ((me._iWidth >>> (nMipMap - 1)) != 1 || (me._iHeight >>> (nMipMap - 1)) != 1)) {
                ...
                if ((!header.ddspf.dwFlags & DDPF_RGB) && (!header.ddspf.dwFlags & DDPF_FOURCC) && (!header.ddspf.dwFlags &DDPF_LUMINANCE)) {
                    debug_error("Флаги DDPF_LUMINANCE, DDPF_RGB или DDPF_FOURCC не выставлены, остальные являются устаревшими");
                }
            ...
            if (iSizeData != header.dwPitchOrLinearSize) {
                ...
                warning("Несовпадают размеры картинки вычисленный и в файле(" + iSizeData + "," +
                ...
                for (k = 0; k < nVolume; k++) {
                    ...
                    for (l = 0; l < 6; l++) {
                        if (TEST_BIT(me._iCubeFlags, a.Img.POSITIVEX + l)) {
                            ...
                            iX = me._iWidth;
                            iY = me._iHeight;
                            iSizeData = (Math.ceil(iX / me.getDivSize()) * Math.ceil(iY / me.getDivSize())) *
                                        me.getBlockBytes();
                            ...
                            for (var b = 0; b < nMipMap; b++) {
                                ...
                                iX = Math.ceil(iX / 2);
                                iY = Math.ceil(iY / 2);
                                iSizeData = Math.ceil((iX * iY) / (me.getDivSize() * me.getDivSize())) *
                                            me.getBlockBytes();
                            }
                        }
                    }
...
Img.prototype.getPixelLA = function (iX, iY, pPixel, iMipLevel, eCubeFlag, iVolumeLevel) {
    ...
    if (eCubeFlag == undefined) {
        ...
        debug_assert(TEST_BIT(this._iFlags, a.Img.CubeMap),
                     "Выставлена часть кубической куртинки, хотя картинка не является кубической");
        debug_assert(TEST_BIT(this._iCubeFlags, eCubeFlag),
                     "Запрашивается часть кубической текстуры которой нет, которого нет");
    }
    ...
    if (iVolumeLevel == undefined) {
        ...
        debug_assert(TEST_BIT(this._iFlags, a.Img.Volume),
                     "Запрашивается часть объемной картинки, а картинка не объемная");
        debug_assert(iVolumeLevel < this.getVolumeLevels(), "Запрашивается часть объемной картинки, которой нет");
    }
...
Img.prototype._getPixelRGBA = function (iX, iY, pPixel, iMipLevel, eCubeFlag, iVolumeLevel) {
    ...
    if (this._eFormat == a.IFORMAT.BGRA8) {
        ...
        iOffset = (Math.ceil(this.getWidth(iMipLevel) / this.getDivSize()) * Math.floor(iY / this.getDivSize()) +
                   Math.floor(iX / this.getDivSize())) * this.getBlockBytes();
        ...
        pPixel[3] = ((pAlpha[Math.floor((this.getDivSize() * iY0 + iX0) / 2)] >>>
                      4 * (((this.getDivSize() * iY0 + iX0) % 2))) & 0xF) * 15;
        if (iCode == 0) {
            pPixel[0] = ((pColor0 & 0xf800) >>> 11) * 8;
            pPixel[1] = ((pColor0 & 0x07e0) >>> 5) * 4;
            pPixel[2] = ((pColor0 & 0x001f)) * 8;
