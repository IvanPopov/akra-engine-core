...
interface RaphaelStatic {
    ...
    bezierBBox(bez: Array<any>): { min: { x: number; y: number; }; max: { x: number; y: number; }; };
   ...
   // getColor: { reset(); };
    ...
    pathIntersection(path1: string, path2: string): { x: number; y: number; t1: number; t2: number; segment1: number; segment2: number; bez1: Array<any>; bez2: Array<any>; }[];
