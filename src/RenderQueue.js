...
RenderEntry.prototype.release = function(){
    this.clear();
    if(this._pAllocator){
...
//    this._nSortCount = 0;
//    this._pSortEntrys = null;
    this._pFreeEntries = null;
    this._pSortEntries = null;
    this._eType = eType;
}
A_NAMESPACE(RenderQueue);
...
//    }
//    this._nSortCount = 0;
    this._pSortEntries = new a.PopArray(nCount, iIncrement);
    this._pFreeEntries = new a.Allocator(a.RenderEntry, nCount, iIncrement);
//    this._pSortEntrys = new a.PopArray(nCount, iIncrement);
    // console.log(this);
};

RenderQueue.prototype.getEmptyEntry = function () {
    return this._pFreeEntries.getElement();
};
...
RenderQueue.prototype.addSortEntry = function (pEntry) {
    this._pSortEntries.push(pEntry);
};

RenderQueue.prototype.execute = function () {
    var i;
    var iLength = this._pSortEntries.length;
    var pData = this._pSortEntries.data;
    var pRenderer = this._pEngine.shaderManager();
    var pEntry;
    for (i = 0; i < iLength; i++) {
        pEntry = pData[i];
