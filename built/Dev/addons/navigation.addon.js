...
(function (akra) {
    (function (_addons) {
        ...
        addons['navigation'].path = addons['navigation'].path || akra.uri.currentPath();

        akra.debug.log("config['addons']['navigation'] = ", JSON.stringify(addons['navigation']));

        function _navigation(pGeneralViewport, pParameters, pCallback) {
            ...
            var pRotationPoint = pParameters.rotationPoint;

            //scene with cube backend
            var pCamera = pScene.createCamera();
            var pLight = pScene.createLightPoint(1 /* PROJECT */, false);
            var pParams = pLight.getParams();
            var pModel = pRmgr.getModelPoolByFormat(4096 /* COLLADA */).findResource("akra.navigation.ORIENTATION_CUBE");
            ...
            var pViewport = pGeneralViewport.getTarget().addViewport((new akra.render.DSViewport(pCamera, 0.7, .05, .25, .25, 100)));
            ...
            //movemenet backend!
            pGeneralViewport.enableSupportForUserEvent(64 /* DRAGSTART */ | 128 /* DRAGSTOP */ | 256 /* DRAGGING */);
            ...
            pViewport.render.connect(function (pViewport, pTechnique, iPass, pRenderable, pSceneObject) {
                ...
                if (pTechnique.isLastPass(iPass)) {
                    ...
                    pPass.setRenderState(19 /* SRCBLEND */, eSrcBlend);
                    pPass.setRenderState(20 /* DESTBLEND */, eDestBlend);
                }
            });

            pViewport.enableSupportForUserEvent(1 /* CLICK */ | 16 /* MOUSEOVER */ | 32 /* MOUSEOUT */ | 64 /* DRAGSTART */ | 128 /* DRAGSTOP */ | 256 /* DRAGGING */ | 512 /* MOUSEWHEEL */);
        ...
        function getNavigationDependences(sPath) {
            var pDep = {"path":"navigation.addon.map","type":"map"};

            return akra.deps.createDependenceByPath(pDep.path, pDep.type, sPath || addons['navigation'].path);
        }
        _addons.getNavigationDependences = getNavigationDependences;
        ...
        */
        function navigation(pGeneralViewport, pParameters, pCallback) {
            if (typeof pParameters === "undefined") { pParameters = null; }
            if (typeof pCallback === "undefined") { pCallback = null; }
            if (akra.isNull(pParameters)) {
            ...
            if (pGeneralViewport.getTarget().getRenderer().getEngine().getResourceManager().getModelPoolByFormat(4096 /* COLLADA */).findResource("akra.navigation.ORIENTATION_CUBE")) {
                return _navigation(pGeneralViewport, pParameters, pCallback);
            }

            akra.deps.load(pGeneralViewport.getTarget().getRenderer().getEngine(), getNavigationDependences(pParameters.path), null, function (e) {
                if (!akra.isNull(e)) {
