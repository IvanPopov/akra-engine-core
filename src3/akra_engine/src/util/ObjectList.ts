/// <reference path="../idl/AIObjectList.ts" />
...
class ObjectList<T> implements AIObjectList<T> {
    ...
    protected _pCurrent: AIObjectListItem<T> = null;
    ...
    indexOf(pData: T, iFrom: uint = 0): int {
        var pItem: AIObjectListItem<T> = this.find(iFrom);
    ...
    mid(iPos: uint = 0, iSize: uint = this._iLength): AIObjectList<T> {
        ...
        var pNewList: AIObjectList<T> = new ObjectList();
        var pItem: AIObjectListItem<T> = this.find(iPos);
    ...
    slice(iStart: uint = 0, iEnd: uint = Math.max(this._iLength - iStart, 0)): AIObjectList<T> {
        return this.mid(iStart, iEnd - iStart);
    }

    move(iFrom: uint, iTo: uint): AIObjectList<T> {
        return this.insert(iTo - 1, this.takeAt(iFrom));
    }

    replace(iPos: uint, pData: T): AIObjectList<T> {
    ...
    swap(i: uint, j: uint): AIObjectList<T> {
        logger.presume(!this.isLocked());
        ...
        if (i != j) {
            var pItem1: AIObjectListItem<T> = this.find(i);
            var pItem2: AIObjectListItem<T> = this.find(j);
    ...
    add(pList: AIObjectList<T>): AIObjectList<T> {
        pList.seek(0);
    ...
    seek(n: int = 0): AIObjectList<T> {
        var pElement: AIObjectListItem<T>;
    ...
    push(pElement: T): AIObjectList<T> {
    ...
    private pullElement(pItem: AIObjectListItem<T>): T {
        if (isNull(pItem)) {
    ...
    prepend(pElement: T): AIObjectList<T> {
			return this.insert(0, pElement)
		}

    private find(n: uint): AIObjectListItem<T> {
        if (n < this._iLength) {
            this.seek(n);
    ...
    private releaseItem(pItem: AIObjectListItem<T>): T {
    ...
    private createItem(): AIObjectListItem<T> {
        if (ObjectList._pool.length == 0) {
            return { next: null, prev: null, data: null }
        }
        return <AIObjectListItem<T>>ObjectList._pool.pop();
    }

    fromArray(elements: T[], iOffset: uint = 0, iSize: uint = elements.length): AIObjectList<T> {
    ...
    insert(n: uint, pData: T): AIObjectList<T> {
        logger.presume(!this.isLocked());

        var pNew: AIObjectListItem<T> = this.createItem();
        var pItem: AIObjectListItem<T>;
    ...
    isEqual(pList: AIObjectList<T>): boolean {
    ...
    clear(): AIObjectList<T> {
        ...
        var pPrev: AIObjectListItem<T>;
        var pNext: AIObjectListItem<T>;
    ...
    forEach(fn: AIListExplorerFunc<T>): void {
        var pItem: AIObjectListItem<T> = this._pHead;
        var n: uint = 0;
        do {
    ...
    private static _pool: AIObjectArray<any> = new ObjectArray<any>();
}
