...
function normalizeURIPath(pFile: AIURI): AIURI {
    if (!isNull(pFile.path)) {
        if (pFile.scheme === "filesystem:") {
            var pUri: AIURI = parse(pFile.path);
...
export function resolve(sFrom: string, sTo: string = document.location.href): string {
    var pCurrentPath: AIURI = parse(sTo);
    var pFile: AIURI = parse(sFrom);
    var sDirname: string;
...
export function parseDataURI(sUri: string): AIDataURI {
...
export function parse(sUri: string): AIURI {
    return new URI(sUri);
}
