...
class ComplexTypeInstruction extends Instruction implements AIAFXTypeInstruction {
    ...
    private _pFieldDeclList: AIAFXVariableDeclInstruction[] = null;
    ...
    constructor() {
        ...
        this._eInstructionType = AEAFXInstructionTypes.k_ComplexTypeInstruction;
    }
    ...
    addField(pVariable: AIAFXVariableDeclInstruction): void {
        ...
        var pType: AIAFXVariableTypeInstruction = pVariable.getType();
        //pType._markAsField();
    ...
    addFields(pFieldCollector: AIAFXInstruction, isSetParent: boolean = true): void {
        this._pFieldDeclList = <AIAFXVariableDeclInstruction[]>(pFieldCollector.getInstructions());
    ...
    _getFieldDeclList(): AIAFXVariableDeclInstruction[] {
    ...
    clone(pRelationMap: AIAFXInstructionMap = <AIAFXInstructionMap>{}): ComplexTypeInstruction {
        ...
        var pFieldDeclList: AIAFXVariableDeclInstruction[] = new Array(this._pFieldDeclList.length);
        var pFieldNameList: string[] = new Array(this._pFieldNameList.length);
        var pFieldDeclMap: AIAFXVariableDeclMap = <AIAFXVariableDeclMap>{};

        for (var i: uint = 0; i < this._pFieldDeclList.length; i++) {
            var pCloneVar: AIAFXVariableDeclInstruction = this._pFieldDeclList[i].clone(pRelationMap);
    ...
    blend(pType: AIAFXTypeInstruction, eMode: AEAFXBlendMode): AIAFXTypeInstruction {
        ...
        if (eMode === AEAFXBlendMode.k_TypeDecl) {
            return null;
        }

        if (eMode === AEAFXBlendMode.k_Uniform || eMode === AEAFXBlendMode.k_Attribute) {
            if (this.hasFieldWithoutSemantic() || pType.hasFieldWithoutSemantic()) {
                return null;
            }
        }

        var pFieldList: AIAFXVariableDeclInstruction[] = this._pFieldDeclList;
        var pBlendType: ComplexTypeInstruction = new ComplexTypeInstruction();
        var pRelationMap: AIAFXInstructionMap = <AIAFXInstructionMap>{};
        ...
        for (var i: uint = 0; i < pFieldList.length; i++) {
            var pField: AIAFXVariableDeclInstruction = pFieldList[i];
            var pBlendField: AIAFXVariableDeclInstruction = null;
            var sFieldName: string = pField.getName();
            var sFieldSemantic: string = pField.getSemantic();

            if (eMode === AEAFXBlendMode.k_Shared) {
            ...
            else if (eMode === AEAFXBlendMode.k_Attribute ||
                eMode === AEAFXBlendMode.k_Uniform ||
                eMode === AEAFXBlendMode.k_VertexOut) {
    ...
    _setCloneFields(pFieldDeclList: AIAFXVariableDeclInstruction[], pFieldNameList: string[],
        pFieldDeclMap: AIAFXVariableDeclMap): void {
    ...
    private analyzeSemantics(): void {
        ...
        for (var i: uint = 0; i < this._pFieldDeclList.length; i++) {
            var pVar: AIAFXVariableDeclInstruction = this._pFieldDeclList[i];
    ...
    private calculatePaddings(): void {
        ...
        for (var i: uint = 0; i < this._pFieldDeclList.length; i++) {
            var pVarType: AIAFXVariableTypeInstruction = this._pFieldDeclList[i].getType();
            var iVarSize: uint = pVarType.getSize();

            if (iVarSize === Instruction.UNDEFINE_SIZE) {
                this.setError(AEEffectErrors.CANNOT_CALCULATE_PADDINGS, { typeName: this.getName() });
                return;
            }
