...
class Mat3 {
    ...
    constructor(m3fMat: AIMat3);
    constructor(m4fMat: AIMat4);
    constructor(pArray: float[]);
    constructor(f1: float, f2: float, f3: float);
    constructor(v3fVec1: AIVec3, v3fVec2: AIVec3, v3fVec3: AIVec3);
    ...
    set(p: float[]): AIMat3;
    /** Put on the diagonal. */
    set(f1: float, f2: float, f3: float): AIMat3;
    /** Arrange the columns. */
    set(v1: AIVec3, v2: AIVec3, v3: AIVec3): AIMat3;
    /** Arrange the columns. */
    set(p1: float[], p2: float[], p3: float[]): AIMat3;
    /** Arrange the columns. */
    set(f1: float, f2: float, f3: float,
        f4: float, f5: float, f6: float,
        f7: float, f8: float, f9: float): AIMat3;
        ...
        if (n == 1) {
            ...
            if (isFloat(arguments[0])) {
                ...
                if (arguments[0] instanceof Vec3) {
                    ...
                    var v3fVec1: AIVec3 = arguments[0];
                    var v3fVec2: AIVec3 = arguments[1];
                    var v3fVec3: AIVec3 = arguments[2];
    ...
    isEqual(m3fMat: AIMat3, fEps: float = 0.): boolean {
        var pData1: Float32Array = this.data;
        var pData2: Float32Array = m3fMat.data;
    ...
    decompose(q4fRotation: AIQuat4, v3fScale: AIVec3): boolean {
        ...
        var m3fRotScale: AIMat3 = this;
        var m3fRotScaleTransposed: AIMat3 = this.transpose(Mat3.temp());
        var isRotScale: boolean = true;

        //понадобятся если порядок умножения был другим
        var m3fScaleRot: AIMat3 = null, m3fScaleRotTransposed: AIMat3 = null;

        //было отражение или нет
        var scaleSign: int = (m3fRotScale.determinant() >= 0.) ? 1 : -1;

        var m3fResult: AIMat3 = Mat3.temp();
        ...
        var m3fInverseScale: AIMat3 = Mat3.temp(1. / x, 1. / y, 1. / z);
    ...
    static fromYawPitchRoll(fYaw: float, fPitch: float, fRoll: float, m3fDestination?: AIMat3): AIMat3;
    static fromYawPitchRoll(v3fAngles: AIVec3, m3fDestination?: AIMat3): AIMat3;
    static fromYawPitchRoll(fYaw?, fPitch?, fRoll?, m3fDestination?): AIMat3 {
    ...
    static fromXYZ(fX: float, fY: float, fZ: float, m3fDestination?: AIMat3): AIMat3;
    static fromXYZ(v3fAngles: AIVec3, m3fDestination?: AIMat3): AIMat3;
    static fromXYZ(fX?, fY?, fZ?, m3fDestination?): AIMat3 {
        if (arguments.length <= 2) {
            //Vec3 + m3fDestination
            var v3fVec: AIVec3 = arguments[0];
    ...
    static temp(): AIMat3;
    static temp(f: float): AIMat3;
    static temp(v: AIVec3): AIMat3;
    static temp(m3: AIMat3): AIMat3;
    static temp(m4: AIMat4): AIMat3;
    static temp(p: float[]): AIMat3;
    static temp(f1: float, f2: float, f3: float): AIMat3;
    static temp(
        v1: AIVec3,
        v2: AIVec3,
        v3: AIVec3): AIMat3;
...
pBuffer = gen.array<AIMat3>(256, Mat3);
iElement = 0;
