...
/// <reference path="../idl/AIPair.ts" />
/// <reference path="../idl/AIColor.ts" />

import bf = require("util/bitflags");
import math = require("math/math");
import PixelBox = require("pixelUtil/PixelBox");
import logger = require("util/logger");
import type = require("type");

import isEmpty = type.isEmpty;

var dynamic_cast_f32_ptr = (uint8_data, n) => (new Float32Array(uint8_data.buffer, uint8_data.byteOffset, n));
var dynamic_cast_u16_ptr = (uint8_data, n) => (new Uint16Array(uint8_data.buffer, uint8_data.byteOffset, n));
var dynamic_cast_u32_ptr = (uint8_data, n) => (new Uint32Array(uint8_data.buffer, uint8_data.byteOffset, n));
var dynamic_cast_i8_ptr = (uint8_data, n) => (new Int8Array(uint8_data.buffer, uint8_data.byteOffset, n));
var dynamic_cast_i16_ptr = (uint8_data, n) => (new Int16Array(uint8_data.buffer, uint8_data.byteOffset, n));
var dynamic_cast_i32_ptr = (uint8_data, n) => (new Int32Array(uint8_data.buffer, uint8_data.byteOffset, n));



export interface AIPixelFormatDescription {
    ...
    */
    ...
    /** Component type
        */
    componentType: AEPixelComponentTypes;
    /** Component count
        */
    componentCount: uint;
...
function fillPixelFormats(pData: any[][]): AIPixelFormatDescription[] {
    var pPixelFormats: AIPixelFormatDescription[] = [];
    ...
    for (var i: int = 0; i < pData.length; ++i) {
        ...
        pPixelFormats.push({
            ...
            flags: <uint>	pEl[2],
            componentType: <AEPixelComponentTypes>pEl[3],
...
var pPixelFormats: AIPixelFormatDescription[] = fillPixelFormats([
    ["PF_UNKNOWN",
    /* Bytes per element */
        0,
    /* Flags */
    ...
    /* Bytes per element */
        1,
        /* Flags */
        AEPixelFormatFlags.LUMINANCE | AEPixelFormatFlags.NATIVEENDIAN,
        /* Component type and count */
        AEPixelComponentTypes.BYTE, 1,
    /* rbits, gbits, bbits, abits */
        8, 0, 0, 0,
    /* Masks and shifts */
        0xFF, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        1,
        /* Flags */
        AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.NATIVEENDIAN,
        /* Component type and count */
        AEPixelComponentTypes.BYTE, 1,
    /* rbits, gbits, bbits, abits */
        0, 0, 0, 8,
    ...
    /* Bytes per element */
        1,
        /* Flags */
        AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.LUMINANCE | AEPixelFormatFlags.NATIVEENDIAN,
        /* Component type and count */
        AEPixelComponentTypes.BYTE, 2,
    /* rbits, gbits, bbits, abits */
        4, 0, 0, 4,
    /* Masks and shifts */
    ...
    /* Bytes per element */
        2,
        /* Flags */
        AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.LUMINANCE,
        /* Component type and count */
        AEPixelComponentTypes.BYTE, 2,
    /* rbits, gbits, bbits, abits */
        8, 0, 0, 8,
    ...
    /* Bytes per element */
        2,
        /* Flags */
        AEPixelFormatFlags.NATIVEENDIAN,
        /* Component type and count */
        AEPixelComponentTypes.BYTE, 3,
    /* rbits, gbits, bbits, abits */
        5, 6, 5, 0,
    /* Masks and shifts */
    ...
    /* Bytes per element */
        0,
        /* Flags */
        AEPixelFormatFlags.COMPRESSED | AEPixelFormatFlags.HASALPHA,
        /* Component type and count */
        AEPixelComponentTypes.BYTE, 3, /* No alpha*/
    /* rbits, gbits, bbits, abits */
        0, 0, 0, 0,
    /* Masks and shifts */
        0, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        6,
        /* Flags */
        AEPixelFormatFlags.FLOAT,
        /* Component type and count */
        AEPixelComponentTypes.FLOAT16, 3,
    /* rbits, gbits, bbits, abits */
        16, 16, 16, 0,
    /* Masks and shifts */
        0, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        8,
        /* Flags */
        AEPixelFormatFlags.FLOAT | AEPixelFormatFlags.HASALPHA,
        /* Component type and count */
        AEPixelComponentTypes.FLOAT16, 4,
    /* rbits, gbits, bbits, abits */
        16, 16, 16, 16,
    /* Masks and shifts */
        0, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        ...
        AEPixelFormatFlags.FLOAT,
        /* Component type and count */
        AEPixelComponentTypes.FLOAT32, 3,
    /* rbits, gbits, bbits, abits */
        32, 32, 32, 0,
    /* Masks and shifts */
        0, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        ...
        AEPixelFormatFlags.FLOAT | AEPixelFormatFlags.HASALPHA,
        /* Component type and count */
        AEPixelComponentTypes.FLOAT32, 4,
    /* rbits, gbits, bbits, abits */
        32, 32, 32, 32,
    /* Masks and shifts */
        0, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        4,
        /* Flags */
        AEPixelFormatFlags.DEPTH,
        /* Component type and count */
        AEPixelComponentTypes.FLOAT32, 1, /* ?*/
    /* rbits, gbits, bbits, abits */
        0, 0, 0, 0,
    /* Masks and shifts */
        0, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        8,
        /* Flags */
        AEPixelFormatFlags.HASALPHA,
        /* Component type and count */
        AEPixelComponentTypes.SHORT, 4,
    /* rbits, gbits, bbits, abits */
        16, 16, 16, 16,
    /* Masks and shifts */
        0, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        ...
        AEPixelFormatFlags.FLOAT,
        /* Component type and count */
        AEPixelComponentTypes.FLOAT16, 1,
    /* rbits, gbits, bbits, abits */
        16, 0, 0, 0,
    /* Masks and shifts */
        0, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        ...
        AEPixelFormatFlags.FLOAT,
        /* Component type and count */
        AEPixelComponentTypes.FLOAT16, 2,
    /* rbits, gbits, bbits, abits */
        16, 16, 0, 0,
    /* Masks and shifts */
        0, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        ...
        AEPixelFormatFlags.FLOAT,
        /* Component type and count */
        AEPixelComponentTypes.FLOAT32, 2,
    /* rbits, gbits, bbits, abits */
        32, 32, 0, 0,
    /* Masks and shifts */
        0, 0, 0, 0, 0, 0, 0, 0
    ...
    /* Bytes per element */
        4,
        /* Flags */
        AEPixelFormatFlags.DEPTH | AEPixelFormatFlags.STENCIL,
        /* Component type and count */
        AEPixelComponentTypes.INT, 1,
    /* rbits, gbits, bbits, abits */
        24, 8, 0, 0,
...
var _pColorValue: AIColorValue = { r: 0., g: 0., b: 0., a: 1. };


export /** inline */ function getDescriptionFor(eFmt: AEPixelFormats): AIPixelFormatDescription {
    var ord: int = <int>eFmt;
    logger.assert(ord >= 0 && ord < AEPixelFormats.TOTAL, "getDescriptionFor:" + ord);
...
*/
export /** inline */ function getNumElemBytes(eFormat: AEPixelFormats): uint {
...
*/
export /** inline */ function getNumElemBits(eFormat: AEPixelFormats): uint {
...
*/
export function getMemorySize(iWidth: uint, iHeight: uint, iDepth: uint, eFormat: AEPixelFormats): uint {
...
*/
export  /** inline */ function getFlags(eFormat: AEPixelFormats): uint {
    return getDescriptionFor(eFormat).flags;
}

/** Shortcut method to determine if the format has an alpha component */
export /** inline */ function hasAlpha(eFormat: AEPixelFormats): boolean {
    return (getFlags(eFormat) & AEPixelFormatFlags.HASALPHA) > 0;
}
/** Shortcut method to determine if the format is floating point */
export /** inline */ function isFloatingPoint(eFormat: AEPixelFormats): boolean {
    return (getFlags(eFormat) & AEPixelFormatFlags.FLOAT) > 0;
}
/** Shortcut method to determine if the format is compressed */
export /** inline */ function isCompressed(eFormat: AEPixelFormats): boolean {
    return (getFlags(eFormat) & AEPixelFormatFlags.COMPRESSED) > 0;
}
/** Shortcut method to determine if the format is a depth format. */
export /** inline */ function isDepth(eFormat: AEPixelFormats): boolean {
    return (getFlags(eFormat) & AEPixelFormatFlags.DEPTH) > 0;
}
/** Shortcut method to determine if the format is in native endian format. */
export /** inline */ function isNativeEndian(eFormat: AEPixelFormats): boolean {
    return (getFlags(eFormat) & AEPixelFormatFlags.NATIVEENDIAN) > 0;
}
/** Shortcut method to determine if the format is a luminance format. */
export /** inline */ function isLuminance(eFormat: AEPixelFormats): boolean {
    return (getFlags(eFormat) & AEPixelFormatFlags.LUMINANCE) > 0;
}
...
*/
export function isValidExtent(iWidth: uint, iHeight: uint, iDepth: uint, eFormat: AEPixelFormats): boolean {
    if (isCompressed(eFormat)) {
        switch (eFormat) {
            case AEPixelFormats.DXT1:
            case AEPixelFormats.DXT2:
...
*/
export function getBitDepths(eFormat: AEPixelFormats): int[] {
    /** @const */ var des: AIPixelFormatDescription = getDescriptionFor(eFormat);
    var rgba: int[] = [];
...
*/
export function getBitMasks(eFormat: AEPixelFormats): uint[] {
    /** @const */ var des: AIPixelFormatDescription = getDescriptionFor(eFormat);
    var rgba: uint[] = [];
...
@note			Only valid for native endian formats
*/
export function getBitShifts(eFormat: AEPixelFormats): uint[] {
    /** @const */ var des: AIPixelFormatDescription = getDescriptionFor(eFormat);
    var rgba: int[] = [];
...
*/
export /** inline */ function getFormatName(eSrcFormat: AEPixelFormats): string {
...
*/
export function isAccessible(eSrcFormat: AEPixelFormats): boolean {
    if (eSrcFormat == AEPixelFormats.UNKNOWN)
        return false;
    var flags: uint = getFlags(eSrcFormat);
    return !((flags & AEPixelFormatFlags.COMPRESSED) || (flags & AEPixelFormatFlags.DEPTH));
}
...
*/
export /** inline */ function getComponentType(eFmt: AEPixelFormats): AEPixelComponentTypes {
 ...
 */
export /** inline */ function getComponentCount(eFmt: AEPixelFormats): uint {
    return getDescriptionFor(eFmt).componentCount;
}

export /** inline */ function getComponentTypeBits(eFormat: AEPixelFormats): uint {
    var eType: AEPixelComponentTypes = getComponentType(eFormat);

    switch (eType) {
        case AEPixelComponentTypes.BYTE: return 8;   /*Byte per component (8 bit fixed 0.0..1.0)*/
        case AEPixelComponentTypes.SHORT: return 16;  /*Short per component (16 bit fixed 0.0..1.0))*/
        case AEPixelComponentTypes.FLOAT16: return 16;  /*16 bit float per component*/
        case AEPixelComponentTypes.FLOAT32: return 32;  /*32 bit float per component*/
    }
...
*/
export function getFormatFromName(sName: string, isAccessibleOnly: boolean = false, isCaseSensitive: boolean = false): AEPixelFormats {
    ...
    for (var i: int = 0; i < AEPixelFormats.TOTAL; ++i) {
        var ePf: AEPixelFormats = <AEPixelFormats>i;
...
/** Gets the BNF expression of the pixel-formats.
    ...
    @param  accessibleOnly  If true, only accessible pixel format will take into account, otherwise all
                            pixel formats list in AEPixelFormats enumeration will being returned.
    @return                A string contains the BNF expression.
*/
export function getBNFExpressionOfPixelFormats(isAccessibleOnly: boolean = false): string {
    // Collect format names sorted by length, it's required by BNF compiler
    // that similar tokens need longer ones comes first.

    var formatNames: AIPair<uint, string>[] = [];
    for (var i: uint = 0; i < AEPixelFormats.TOTAL; ++i) {
        var ePf: AEPixelFormats = <AEPixelFormats>(i);
        if (!isAccessibleOnly || isAccessible(ePf)) {
...
*/
export function getFormatForBitDepths(eFmt: AEPixelFormats, iIntegerBits: uint, iFloatBits: uint): AEPixelFormats {
    switch (iIntegerBits) {
        case 16:
            switch (eFmt) {
                case AEPixelFormats.R8G8B8:
                case AEPixelFormats.X8R8G8B8:
                    return AEPixelFormats.R5G6B5;
        ...
        case 16:
            switch (eFmt) {
                case AEPixelFormats.FLOAT32_R:
                    return AEPixelFormats.FLOAT16_R;

                case AEPixelFormats.FLOAT32_RGB:
                    return AEPixelFormats.FLOAT16_RGB;

                case AEPixelFormats.FLOAT32_RGBA:
                    return AEPixelFormats.FLOAT16_RGBA;
        ...
        case 32:
            switch (eFmt) {
                case AEPixelFormats.FLOAT16_R:
                    return AEPixelFormats.FLOAT32_R;

                case AEPixelFormats.FLOAT16_RGB:
                    return AEPixelFormats.FLOAT32_RGB;
...
/** Pack a colour value to memory
    ...
    @param dest		Destination memory location
*/
export function packColourUint(r: uint, g: uint, b: uint, a: uint, ePf: AEPixelFormats, pDest: Uint8Array): void {
    // if (arguments.length < 4) {
    // 	var cColour: AIColor = arguments[0];
    // 	packColour(cColour.r, cColour.g, cColour.b, cColour.a, ePf, pDest);
    // 	return;
    // }

    /** @const */ var des: AIPixelFormatDescription = getDescriptionFor(ePf);
    if (des.flags & AEPixelFormatFlags.NATIVEENDIAN) {
...
/** Pack a colour value to memory
   ...
   @param dest		Destination memory location
*/
export function packColourFloat(r: float, g: float, b: float, a: float, ePf: AEPixelFormats, pDest: Uint8Array): void {
    // Catch-it-all here
    /** @const */ var des: AIPixelFormatDescription = getDescriptionFor(ePf);
    if (des.flags & AEPixelFormatFlags.NATIVEENDIAN) {
        // Do the packing
        //std::cerr << dest << " " << r << " " << g <<  " " << b << " " << a << std::endl;
        /** @const */ var value: uint = ((bf.floatToFixed(r, des.rbits) << des.rshift) & des.rmask) |
...
*/
export function unpackColour(cColour: AIColor, ePf: AEPixelFormats, pSrc: Uint8Array): void {
...
/** Unpack a colour value from memory
    ...
    @remarks 	This function returns the colour components in 8 bit precision,
        ...
        point formats.  
*/
export function unpackColourUint(rgba: AIColorIValue, ePf: AEPixelFormats, pSrc: Uint8Array): void {
    /** @const */ var des: AIPixelFormatDescription = getDescriptionFor(ePf);
    var r: uint = 0, g: uint = 0, b: uint = 0, a: uint = 0;

    if (des.flags & AEPixelFormatFlags.NATIVEENDIAN) {
        // Shortcut for integer formats unpacking
        /** @const */ var value: uint = bf.intRead(pSrc, des.elemBytes);
        if (des.flags & AEPixelFormatFlags.LUMINANCE) {
        ...
        if (des.flags & AEPixelFormatFlags.HASALPHA) {
        ...
        var pRGBA: AIColorValue = _pColorValue;
...
/** Unpack a colour value from memory
    ...
    @param src		Source memory location
*/
export function unpackColourFloat(rgba: AIColorValue, ePf: AEPixelFormats, pSrc: Uint8Array): void {
    /** @const */ var des: AIPixelFormatDescription = getDescriptionFor(ePf);
 ...
 */
export function bulkPixelConversion(pSrc: Uint8Array, eSrcFormat: AEPixelFormats, pDest: Uint8Array, eDstFormat: AEPixelFormats, iCount: uint): void;
...
export function bulkPixelConversion(pSrc: AIPixelBox, pDest: AIPixelBox): void;

export function bulkPixelConversion(pSrc: any, eSrcFormat: any, pDest?: any, eDstFormat?: any, iCount?: uint): void {
    var src: AIPixelBox = null,
        dst: AIPixelBox = null;

    if (arguments.length > 2) {
        src = new PixelBox(iCount, 1, 1, <AEPixelFormats>eSrcFormat, <Uint8Array>pSrc);
        dst = new PixelBox(iCount, 1, 1, <AEPixelFormats>eDstFormat, <Uint8Array>pDest);
    }
    else {
        src = arguments[0];
        dst = arguments[1];
    ...
    if (src.width !== dst.width || src.height !== dst.height || src.depth !== dst.depth) {
        logger.critical("Size dest and src pictures is different");
        return;
    }
    ...
    if (isCompressed(src.format) || isCompressed(dst.format)) {
        if (src.format == dst.format) {
            ...
            logger.error("This method can not be used to compress or decompress images",
    ...
    if (src.format == dst.format) {
        ...
        /** @const */ var rowSize: uint = src.width * srcPixelSize;
    ...
    if (dst.format == AEPixelFormats.X8R8G8B8 || dst.format == AEPixelFormats.X8B8G8R8) {
        // Do the same conversion, with AEPixelFormats.A8R8G8B8, which has a lot of
        // optimized conversions
        var tempdst: AIPixelBox = dst;
        tempdst.format = (dst.format == AEPixelFormats.X8R8G8B8) ? AEPixelFormats.A8R8G8B8 : AEPixelFormats.A8B8G8R8;
        bulkPixelConversion(src, tempdst);
        return;
    }


    // Converting from AEPixelFormats.X8R8G8B8 is exactly the same as converting from
    // AEPixelFormats.A8R8G8B8, given that the destination format does not have alpha.
    if ((src.format == AEPixelFormats.X8R8G8B8 || src.format == AEPixelFormats.X8B8G8R8) && !hasAlpha(dst.format)) {
        // Do the same conversion, with AEPixelFormats.A8R8G8B8, which has a lot of
        // optimized conversions
        var tempsrc: AIPixelBox = src;
        tempsrc.format = src.format == AEPixelFormats.X8R8G8B8 ? AEPixelFormats.A8R8G8B8 : AEPixelFormats.A8B8G8R8;
