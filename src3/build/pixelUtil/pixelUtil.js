/// <reference path="../idl/AEPixelFormats.ts" />
/// <reference path="../idl/AIPair.ts" />
/// <reference path="../idl/AIColor.ts" />
define(["require", "exports", "util/bitflags", "math/math", "pixelUtil/PixelBox", "util/logger"], function(require, exports, __bf__, __math__, __PixelBox__, __logger__) {
    var bf = __bf__;
    var math = __math__;
    var PixelBox = "pixelUtil/PixelBox";
    var logger = __logger__;
    

    var isEmpty = type.isEmpty;

    var dynamic_cast_f32_ptr = function (uint8_data, n) {
        return (new Float32Array(uint8_data.buffer, uint8_data.byteOffset, n));
    };
    var dynamic_cast_u16_ptr = function (uint8_data, n) {
        return (new Uint16Array(uint8_data.buffer, uint8_data.byteOffset, n));
    };
    var dynamic_cast_u32_ptr = function (uint8_data, n) {
        return (new Uint32Array(uint8_data.buffer, uint8_data.byteOffset, n));
    };
    var dynamic_cast_i8_ptr = function (uint8_data, n) {
        return (new Int8Array(uint8_data.buffer, uint8_data.byteOffset, n));
    };
    var dynamic_cast_i16_ptr = function (uint8_data, n) {
        return (new Int16Array(uint8_data.buffer, uint8_data.byteOffset, n));
    };
    var dynamic_cast_i32_ptr = function (uint8_data, n) {
        return (new Int32Array(uint8_data.buffer, uint8_data.byteOffset, n));
    };

    function fillPixelFormats(pData) {
        var pPixelFormats = [];

        for (var i = 0; i < pData.length; ++i) {
            var pEl = pData[i];
            pPixelFormats.push({
                name: pEl[0],
                elemBytes: pEl[1],
                flags: pEl[2],
                componentType: pEl[3],
                componentCount: pEl[4],
                rbits: pEl[5],
                gbits: pEl[6],
                bbits: pEl[7],
                abits: pEl[8],
                rmask: pEl[9],
                gmask: pEl[10],
                bmask: pEl[11],
                amask: pEl[12],
                rshift: pEl[13],
                gshift: pEl[14],
                bshift: pEl[15],
                ashift: pEl[16]
            });
        }

        return pPixelFormats;
    }

    var pPixelFormats = fillPixelFormats([
        [
            "PF_UNKNOWN",
            /* Bytes per element */
            0,
            /* Flags */
            0,
            AEPixelComponentTypes.BYTE,
            0,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_L8",
            /* Bytes per element */
            1,
            AEPixelFormatFlags.LUMINANCE | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            1,
            /* rbits, gbits, bbits, abits */
            8,
            0,
            0,
            0,
            /* Masks and shifts */
            0xFF,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_L16",
            /* Bytes per element */
            2,
            AEPixelFormatFlags.LUMINANCE | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.SHORT,
            1,
            /* rbits, gbits, bbits, abits */
            16,
            0,
            0,
            0,
            /* Masks and shifts */
            0xFFFF,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_A8",
            /* Bytes per element */
            1,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            1,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            8,
            /* Masks and shifts */
            0,
            0,
            0,
            0xFF,
            0,
            0,
            0,
            0
        ],
        [
            "PF_A4L4",
            /* Bytes per element */
            1,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.LUMINANCE | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            2,
            /* rbits, gbits, bbits, abits */
            4,
            0,
            0,
            4,
            /* Masks and shifts */
            0x0F,
            0,
            0,
            0xF0,
            0,
            0,
            0,
            4
        ],
        [
            "PF_BYTE_LA",
            /* Bytes per element */
            2,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.LUMINANCE,
            AEPixelComponentTypes.BYTE,
            2,
            /* rbits, gbits, bbits, abits */
            8,
            0,
            0,
            8,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_R5G6B5",
            /* Bytes per element */
            2,
            AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            3,
            /* rbits, gbits, bbits, abits */
            5,
            6,
            5,
            0,
            /* Masks and shifts */
            0xF800,
            0x07E0,
            0x001F,
            0,
            11,
            5,
            0,
            0
        ],
        [
            "PF_B5G6R5",
            /* Bytes per element */
            2,
            AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            3,
            /* rbits, gbits, bbits, abits */
            5,
            6,
            5,
            0,
            /* Masks and shifts */
            0x001F,
            0x07E0,
            0xF800,
            0,
            0,
            5,
            11,
            0
        ],
        [
            "PF_A4R4G4B4",
            /* Bytes per element */
            2,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            4,
            4,
            4,
            4,
            /* Masks and shifts */
            0x0F00,
            0x00F0,
            0x000F,
            0xF000,
            8,
            4,
            0,
            12
        ],
        [
            "PF_A1R5G5B5",
            /* Bytes per element */
            2,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            5,
            5,
            5,
            1,
            /* Masks and shifts */
            0x7C00,
            0x03E0,
            0x001F,
            0x8000,
            10,
            5,
            0,
            15
        ],
        [
            "PF_R8G8B8",
            /* Bytes per element */
            3,
            AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            3,
            /* rbits, gbits, bbits, abits */
            8,
            8,
            8,
            0,
            /* Masks and shifts */
            0xFF0000,
            0x00FF00,
            0x0000FF,
            0,
            16,
            8,
            0,
            0
        ],
        [
            "PF_B8G8R8",
            /* Bytes per element */
            3,
            AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            3,
            /* rbits, gbits, bbits, abits */
            8,
            8,
            8,
            0,
            /* Masks and shifts */
            0x0000FF,
            0x00FF00,
            0xFF0000,
            0,
            0,
            8,
            16,
            0
        ],
        [
            "PF_A8R8G8B8",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            8,
            8,
            8,
            8,
            /* Masks and shifts */
            0x00FF0000,
            0x0000FF00,
            0x000000FF,
            0xFF000000,
            16,
            8,
            0,
            24
        ],
        [
            "PF_A8B8G8R8",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            8,
            8,
            8,
            8,
            /* Masks and shifts */
            0x000000FF,
            0x0000FF00,
            0x00FF0000,
            0xFF000000,
            0,
            8,
            16,
            24
        ],
        [
            "PF_B8G8R8A8",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            8,
            8,
            8,
            8,
            /* Masks and shifts */
            0x0000FF00,
            0x00FF0000,
            0xFF000000,
            0x000000FF,
            8,
            16,
            24,
            0
        ],
        [
            "PF_A2R10G10B10",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            10,
            10,
            10,
            2,
            /* Masks and shifts */
            0x3FF00000,
            0x000FFC00,
            0x000003FF,
            0xC0000000,
            20,
            10,
            0,
            30
        ],
        [
            "PF_A2B10G10R10",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            10,
            10,
            10,
            2,
            /* Masks and shifts */
            0x000003FF,
            0x000FFC00,
            0x3FF00000,
            0xC0000000,
            0,
            10,
            20,
            30
        ],
        [
            "PF_DXT1",
            /* Bytes per element */
            0,
            AEPixelFormatFlags.COMPRESSED | AEPixelFormatFlags.HASALPHA,
            AEPixelComponentTypes.BYTE,
            3,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_DXT2",
            /* Bytes per element */
            0,
            AEPixelFormatFlags.COMPRESSED | AEPixelFormatFlags.HASALPHA,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_DXT3",
            /* Bytes per element */
            0,
            AEPixelFormatFlags.COMPRESSED | AEPixelFormatFlags.HASALPHA,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_DXT4",
            /* Bytes per element */
            0,
            AEPixelFormatFlags.COMPRESSED | AEPixelFormatFlags.HASALPHA,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_DXT5",
            /* Bytes per element */
            0,
            AEPixelFormatFlags.COMPRESSED | AEPixelFormatFlags.HASALPHA,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_FLOAT16_RGB",
            /* Bytes per element */
            6,
            AEPixelFormatFlags.FLOAT,
            AEPixelComponentTypes.FLOAT16,
            3,
            /* rbits, gbits, bbits, abits */
            16,
            16,
            16,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_FLOAT16_RGBA",
            /* Bytes per element */
            8,
            AEPixelFormatFlags.FLOAT | AEPixelFormatFlags.HASALPHA,
            AEPixelComponentTypes.FLOAT16,
            4,
            /* rbits, gbits, bbits, abits */
            16,
            16,
            16,
            16,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_FLOAT32_RGB",
            /* Bytes per element */
            12,
            AEPixelFormatFlags.FLOAT,
            AEPixelComponentTypes.FLOAT32,
            3,
            /* rbits, gbits, bbits, abits */
            32,
            32,
            32,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_FLOAT32_RGBA",
            /* Bytes per element */
            16,
            AEPixelFormatFlags.FLOAT | AEPixelFormatFlags.HASALPHA,
            AEPixelComponentTypes.FLOAT32,
            4,
            /* rbits, gbits, bbits, abits */
            32,
            32,
            32,
            32,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_X8R8G8B8",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            3,
            /* rbits, gbits, bbits, abits */
            8,
            8,
            8,
            0,
            /* Masks and shifts */
            0x00FF0000,
            0x0000FF00,
            0x000000FF,
            0xFF000000,
            16,
            8,
            0,
            24
        ],
        [
            "PF_X8B8G8R8",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            3,
            /* rbits, gbits, bbits, abits */
            8,
            8,
            8,
            0,
            /* Masks and shifts */
            0x000000FF,
            0x0000FF00,
            0x00FF0000,
            0xFF000000,
            0,
            8,
            16,
            24
        ],
        [
            "PF_R8G8B8A8",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.HASALPHA | AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            8,
            8,
            8,
            8,
            /* Masks and shifts */
            0xFF000000,
            0x00FF0000,
            0x0000FF00,
            0x000000FF,
            24,
            16,
            8,
            0
        ],
        [
            "PF_FLOAT32_DEPTH",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.DEPTH,
            AEPixelComponentTypes.FLOAT32,
            1,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_SHORT_RGBA",
            /* Bytes per element */
            8,
            AEPixelFormatFlags.HASALPHA,
            AEPixelComponentTypes.SHORT,
            4,
            /* rbits, gbits, bbits, abits */
            16,
            16,
            16,
            16,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_R3G3B2",
            /* Bytes per element */
            1,
            AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            3,
            /* rbits, gbits, bbits, abits */
            3,
            3,
            2,
            0,
            /* Masks and shifts */
            0xE0,
            0x1C,
            0x03,
            0,
            5,
            2,
            0,
            0
        ],
        [
            "PF_FLOAT16_R",
            /* Bytes per element */
            2,
            AEPixelFormatFlags.FLOAT,
            AEPixelComponentTypes.FLOAT16,
            1,
            /* rbits, gbits, bbits, abits */
            16,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_FLOAT32_R",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.FLOAT,
            AEPixelComponentTypes.FLOAT32,
            1,
            /* rbits, gbits, bbits, abits */
            32,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_SHORT_GR",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.SHORT,
            2,
            /* rbits, gbits, bbits, abits */
            16,
            16,
            0,
            0,
            /* Masks and shifts */
            0x0000FFFF,
            0xFFFF0000,
            0,
            0,
            0,
            16,
            0,
            0
        ],
        [
            "PF_FLOAT16_GR",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.FLOAT,
            AEPixelComponentTypes.FLOAT16,
            2,
            /* rbits, gbits, bbits, abits */
            16,
            16,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_FLOAT32_GR",
            /* Bytes per element */
            8,
            AEPixelFormatFlags.FLOAT,
            AEPixelComponentTypes.FLOAT32,
            2,
            /* rbits, gbits, bbits, abits */
            32,
            32,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_SHORT_RGB",
            /* Bytes per element */
            6,
            /* Flags */
            0,
            AEPixelComponentTypes.SHORT,
            3,
            /* rbits, gbits, bbits, abits */
            16,
            16,
            16,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_PVRTC_RGB2",
            /* Bytes per element */
            0,
            AEPixelFormatFlags.COMPRESSED,
            AEPixelComponentTypes.BYTE,
            3,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_PVRTC_RGBA2",
            /* Bytes per element */
            0,
            AEPixelFormatFlags.COMPRESSED | AEPixelFormatFlags.HASALPHA,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_PVRTC_RGB4",
            /* Bytes per element */
            0,
            AEPixelFormatFlags.COMPRESSED,
            AEPixelComponentTypes.BYTE,
            3,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_PVRTC_RGBA4",
            /* Bytes per element */
            0,
            AEPixelFormatFlags.COMPRESSED | AEPixelFormatFlags.HASALPHA,
            AEPixelComponentTypes.BYTE,
            4,
            /* rbits, gbits, bbits, abits */
            0,
            0,
            0,
            0,
            /* Masks and shifts */
            0,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_R8",
            /* Bytes per element */
            1,
            AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            1,
            /* rbits, gbits, bbits, abits */
            8,
            0,
            0,
            0,
            /* Masks and shifts */
            0xFF0000,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_RG8",
            /* Bytes per element */
            2,
            AEPixelFormatFlags.NATIVEENDIAN,
            AEPixelComponentTypes.BYTE,
            2,
            /* rbits, gbits, bbits, abits */
            8,
            8,
            0,
            0,
            /* Masks and shifts */
            0xFF0000,
            0x00FF00,
            0,
            0,
            8,
            0,
            0,
            0
        ],
        [
            "PF_DEPTH_BYTE",
            /* Bytes per element */
            1,
            AEPixelFormatFlags.DEPTH,
            AEPixelComponentTypes.BYTE,
            1,
            /* rbits, gbits, bbits, abits */
            8,
            0,
            0,
            0,
            /* Masks and shifts */
            0xFF,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_DEPTH_SHORT",
            /* Bytes per element */
            2,
            AEPixelFormatFlags.DEPTH,
            AEPixelComponentTypes.SHORT,
            1,
            /* rbits, gbits, bbits, abits */
            16,
            0,
            0,
            0,
            /* Masks and shifts */
            0xFFFF,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_DEPTH_INT",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.DEPTH,
            AEPixelComponentTypes.INT,
            1,
            /* rbits, gbits, bbits, abits */
            32,
            0,
            0,
            0,
            /* Masks and shifts */
            0xFFFFFFFF,
            0,
            0,
            0,
            0,
            0,
            0,
            0
        ],
        [
            "PF_DEPTH24STENCIL8",
            /* Bytes per element */
            4,
            AEPixelFormatFlags.DEPTH | AEPixelFormatFlags.STENCIL,
            AEPixelComponentTypes.INT,
            1,
            /* rbits, gbits, bbits, abits */
            24,
            8,
            0,
            0,
            /* Masks and shifts */
            0x00FFFFFF,
            0xFF000000,
            0,
            0,
            0,
            24,
            0,
            0
        ]
    ]);

    var _pColorValue = { r: 0., g: 0., b: 0., a: 1. };

    function getDescriptionFor(eFmt) {
        var ord = eFmt;
        logger.assert(ord >= 0 && ord < AEPixelFormats.TOTAL, "getDescriptionFor:" + ord);

        return pPixelFormats[ord];
    }
    exports.getDescriptionFor = getDescriptionFor;

    /** Returns the size in bytes of an element of the given pixel format.
    @return
    The size in bytes of an element. See Remarks.
    @remarks
    Passing PF_UNKNOWN will result in returning a size of 0 bytes.
    */
    function getNumElemBytes(eFormat) {
        return exports.getDescriptionFor(eFormat).elemBytes;
    }
    exports.getNumElemBytes = getNumElemBytes;

    /** Returns the size in bits of an element of the given pixel format.
    @return
    The size in bits of an element. See Remarks.
    @remarks
    Passing PF_UNKNOWN will result in returning a size of 0 bits.
    */
    function getNumElemBits(eFormat) {
        return exports.getDescriptionFor(eFormat).elemBytes * 8;
    }
    exports.getNumElemBits = getNumElemBits;

    /** Returns the size in memory of a region with the given extents and pixel
    format with consecutive memory layout.
    @param width
    The width of the area
    @param height
    The height of the area
    @param depth
    The depth of the area
    @param format
    The format of the area
    @return
    The size in bytes
    @remarks
    In case that the format is non-compressed, this simply returns
    width*height*depth*PixelUtil::getNumElemBytes(format). In the compressed
    case, this does serious magic.
    */
    function getMemorySize(iWidth, iHeight, iDepth, eFormat) {
        if (exports.isCompressed(eFormat)) {
            switch (eFormat) {
                case AEPixelFormats.DXT1:
                    return Math.floor((iWidth + 3) / 4) * Math.floor((iHeight + 3) / 4) * 8 * iDepth;
                case AEPixelFormats.DXT2:
                case AEPixelFormats.DXT3:
                case AEPixelFormats.DXT4:
                case AEPixelFormats.DXT5:
                    return Math.floor((iWidth + 3) / 4) * Math.floor((iHeight + 3) / 4) * 16 * iDepth;

                case AEPixelFormats.PVRTC_RGB2:
                case AEPixelFormats.PVRTC_RGBA2:
                    logger.assert(iDepth == 1);
                    return (math.max(iWidth, 16) * math.max(iHeight, 8) * 2 + 7) / 8;
                case AEPixelFormats.PVRTC_RGB4:
                case AEPixelFormats.PVRTC_RGBA4:
                    logger.assert(iDepth == 1);
                    return (math.max(iWidth, 8) * math.max(iHeight, 8) * 4 + 7) / 8;
                default:
                    logger.error("Invalid compressed pixel format", "PixelUtil::getMemorySize");
            }
        } else {
            return iWidth * iHeight * iDepth * exports.getNumElemBytes(eFormat);
        }
    }
    exports.getMemorySize = getMemorySize;

    /** Returns the property flags for this pixel format
    @return
    A bitfield combination of PFF_HASALPHA, PFF_ISCOMPRESSED,
    PFF_FLOAT, PFF_DEPTH, PFF_NATIVEENDIAN, PFF_LUMINANCE
    @remarks
    This replaces the separate functions for formatHasAlpha, formatIsFloat, ...
    */
    function getFlags(eFormat) {
        return exports.getDescriptionFor(eFormat).flags;
    }
    exports.getFlags = getFlags;

    /** Shortcut method to determine if the format has an alpha component */
    function hasAlpha(eFormat) {
        return (exports.getFlags(eFormat) & AEPixelFormatFlags.HASALPHA) > 0;
    }
    exports.hasAlpha = hasAlpha;

    /** Shortcut method to determine if the format is floating point */
    function isFloatingPoint(eFormat) {
        return (exports.getFlags(eFormat) & AEPixelFormatFlags.FLOAT) > 0;
    }
    exports.isFloatingPoint = isFloatingPoint;

    /** Shortcut method to determine if the format is compressed */
    function isCompressed(eFormat) {
        return (exports.getFlags(eFormat) & AEPixelFormatFlags.COMPRESSED) > 0;
    }
    exports.isCompressed = isCompressed;

    /** Shortcut method to determine if the format is a depth format. */
    function isDepth(eFormat) {
        return (exports.getFlags(eFormat) & AEPixelFormatFlags.DEPTH) > 0;
    }
    exports.isDepth = isDepth;

    /** Shortcut method to determine if the format is in native endian format. */
    function isNativeEndian(eFormat) {
        return (exports.getFlags(eFormat) & AEPixelFormatFlags.NATIVEENDIAN) > 0;
    }
    exports.isNativeEndian = isNativeEndian;

    /** Shortcut method to determine if the format is a luminance format. */
    function isLuminance(eFormat) {
        return (exports.getFlags(eFormat) & AEPixelFormatFlags.LUMINANCE) > 0;
    }
    exports.isLuminance = isLuminance;

    /** Return wether a certain image extent is valid for this image format.
    @param width
    The width of the area
    @param height
    The height of the area
    @param depth
    The depth of the area
    @param format
    The format of the area
    @remarks For non-compressed formats, this is always true. For DXT formats,
    only sizes with a width and height multiple of 4 and depth 1 are allowed.
    */
    function isValidExtent(iWidth, iHeight, iDepth, eFormat) {
        if (exports.isCompressed(eFormat)) {
            switch (eFormat) {
                case AEPixelFormats.DXT1:
                case AEPixelFormats.DXT2:
                case AEPixelFormats.DXT3:
                case AEPixelFormats.DXT4:
                case AEPixelFormats.DXT5:
                    return ((iWidth & 3) == 0 && (iHeight & 3) == 0 && iDepth == 1);
                default:
                    return true;
            }
        } else {
            return true;
        }
    }
    exports.isValidExtent = isValidExtent;

    /** Gives the number of bits (RGBA) for a format. See remarks.
    @remarks      For non-colour formats (dxt, depth) this returns [0,0,0,0].
    */
    function getBitDepths(eFormat) {
        /** @const */ var des = exports.getDescriptionFor(eFormat);
        var rgba = [];

        rgba[0] = des.rbits;
        rgba[1] = des.gbits;
        rgba[2] = des.bbits;
        rgba[3] = des.abits;

        return rgba;
    }
    exports.getBitDepths = getBitDepths;

    /** Gives the masks for the R, G, B and A component
    @note			Only valid for native endian formats
    */
    function getBitMasks(eFormat) {
        /** @const */ var des = exports.getDescriptionFor(eFormat);
        var rgba = [];

        rgba[0] = des.rmask;
        rgba[1] = des.gmask;
        rgba[2] = des.bmask;
        rgba[3] = des.amask;

        return rgba;
    }
    exports.getBitMasks = getBitMasks;

    /** Gives the bit shifts for R, G, B and A component
    @note			Only valid for native endian formats
    */
    function getBitShifts(eFormat) {
        /** @const */ var des = exports.getDescriptionFor(eFormat);
        var rgba = [];

        rgba[0] = des.rshift;
        rgba[1] = des.gshift;
        rgba[2] = des.bshift;
        rgba[3] = des.ashift;

        return rgba;
    }
    exports.getBitShifts = getBitShifts;

    /** Gets the name of an image format
    */
    function getFormatName(eSrcFormat) {
        return exports.getDescriptionFor(eSrcFormat).name;
    }
    exports.getFormatName = getFormatName;

    /** Returns wether the format can be packed or unpacked with the packColour()
    and unpackColour() functions. This is generally not true for compressed and
    depth formats as they are special. It can only be true for formats with a
    fixed element size.
    @return
    true if yes, otherwise false
    */
    function isAccessible(eSrcFormat) {
        if (eSrcFormat == AEPixelFormats.UNKNOWN)
            return false;
        var flags = exports.getFlags(eSrcFormat);
        return !((flags & AEPixelFormatFlags.COMPRESSED) || (flags & AEPixelFormatFlags.DEPTH));
    }
    exports.isAccessible = isAccessible;

    /** Returns the component type for a certain pixel format. Returns PCT_BYTE
    in case there is no clear component type like with compressed formats.
    This is one of PCT_BYTE, PCT_SHORT, PCT_FLOAT16, PCT_FLOAT32.
    */
    function getComponentType(eFmt) {
        return exports.getDescriptionFor(eFmt).componentType;
    }
    exports.getComponentType = getComponentType;

    /** Returns the component count for a certain pixel format. Returns 3(no alpha) or
    4 (has alpha) in case there is no clear component type like with compressed formats.
    */
    function getComponentCount(eFmt) {
        return exports.getDescriptionFor(eFmt).componentCount;
    }
    exports.getComponentCount = getComponentCount;

    function getComponentTypeBits(eFormat) {
        var eType = exports.getComponentType(eFormat);

        switch (eType) {
            case AEPixelComponentTypes.BYTE:
                return 8;
            case AEPixelComponentTypes.SHORT:
                return 16;
            case AEPixelComponentTypes.FLOAT16:
                return 16;
            case AEPixelComponentTypes.FLOAT32:
                return 32;
        }

        return 0;
    }
    exports.getComponentTypeBits = getComponentTypeBits;

    /** Gets the format from given name.
    @param  name            The string of format name
    @param  accessibleOnly  If true, non-accessible format will treat as invalid format,
    otherwise, all supported format are valid.
    @param  caseSensitive   Should be set true if string match should use case sensitivity.
    @return                The format match the format name, or PF_UNKNOWN if is invalid name.
    */
    function getFormatFromName(sName, isAccessibleOnly, isCaseSensitive) {
        if (typeof isAccessibleOnly === "undefined") { isAccessibleOnly = false; }
        if (typeof isCaseSensitive === "undefined") { isCaseSensitive = false; }
        var tmp = sName;

        if (!isCaseSensitive) {
            // We are stored upper-case format names.
            tmp = tmp.toUpperCase();
        }

        for (var i = 0; i < AEPixelFormats.TOTAL; ++i) {
            var ePf = i;
            if (!isAccessibleOnly || exports.isAccessible(ePf)) {
                if (tmp == exports.getFormatName(ePf))
                    return ePf;
            }
        }

        return AEPixelFormats.UNKNOWN;
    }
    exports.getFormatFromName = getFormatFromName;

    /** Gets the BNF expression of the pixel-formats.
    @note                   The string returned by this function is intended to be used as a BNF expression
    to work with Compiler2Pass.
    @param  accessibleOnly  If true, only accessible pixel format will take into account, otherwise all
    pixel formats list in AEPixelFormats enumeration will being returned.
    @return                A string contains the BNF expression.
    */
    function getBNFExpressionOfPixelFormats(isAccessibleOnly) {
        if (typeof isAccessibleOnly === "undefined") { isAccessibleOnly = false; }
        // Collect format names sorted by length, it's required by BNF compiler
        // that similar tokens need longer ones comes first.
        var formatNames = [];
        for (var i = 0; i < AEPixelFormats.TOTAL; ++i) {
            var ePf = (i);
            if (!isAccessibleOnly || exports.isAccessible(ePf)) {
                var formatName = exports.getFormatName(ePf);
                formatNames.push({ first: formatName.length, second: formatName });
            }
        }

        // Populate the BNF expression in reverse order
        var result = "";

        for (var j in formatNames) {
            if (!isEmpty(result))
                result += " | ";
            result += "'" + formatNames[j] + "'";
        }

        return result;
    }
    exports.getBNFExpressionOfPixelFormats = getBNFExpressionOfPixelFormats;

    /** Returns the similar format but acoording with given bit depths.
    @param fmt      The original foamt.
    @param integerBits Preferred bit depth (pixel bits) for integer pixel format.
    Available values: 0, 16 and 32, where 0 (the default) means as it is.
    @param floatBits Preferred bit depth (channel bits) for float pixel format.
    Available values: 0, 16 and 32, where 0 (the default) means as it is.
    @return        The format that similar original format with bit depth according
    with preferred bit depth, or original format if no conversion occurring.
    */
    function getFormatForBitDepths(eFmt, iIntegerBits, iFloatBits) {
        switch (iIntegerBits) {
            case 16:
                switch (eFmt) {
                    case AEPixelFormats.R8G8B8:
                    case AEPixelFormats.X8R8G8B8:
                        return AEPixelFormats.R5G6B5;

                    case AEPixelFormats.B8G8R8:
                    case AEPixelFormats.X8B8G8R8:
                        return AEPixelFormats.B5G6R5;

                    case AEPixelFormats.A8R8G8B8:
                    case AEPixelFormats.R8G8B8A8:
                    case AEPixelFormats.A8B8G8R8:
                    case AEPixelFormats.B8G8R8A8:
                        return AEPixelFormats.A4R4G4B4;

                    case AEPixelFormats.A2R10G10B10:
                    case AEPixelFormats.A2B10G10R10:
                        return AEPixelFormats.A1R5G5B5;

                    default:
                        break;
                }
                break;

            case 32:
                switch (eFmt) {
                    case AEPixelFormats.R5G6B5:
                        return AEPixelFormats.X8R8G8B8;

                    case AEPixelFormats.B5G6R5:
                        return AEPixelFormats.X8B8G8R8;

                    case AEPixelFormats.A4R4G4B4:
                        return AEPixelFormats.A8R8G8B8;

                    case AEPixelFormats.A1R5G5B5:
                        return AEPixelFormats.A2R10G10B10;

                    default:
                        break;
                }
                break;

            default:
                break;
        }

        switch (iFloatBits) {
            case 16:
                switch (eFmt) {
                    case AEPixelFormats.FLOAT32_R:
                        return AEPixelFormats.FLOAT16_R;

                    case AEPixelFormats.FLOAT32_RGB:
                        return AEPixelFormats.FLOAT16_RGB;

                    case AEPixelFormats.FLOAT32_RGBA:
                        return AEPixelFormats.FLOAT16_RGBA;

                    default:
                        break;
                }
                break;

            case 32:
                switch (eFmt) {
                    case AEPixelFormats.FLOAT16_R:
                        return AEPixelFormats.FLOAT32_R;

                    case AEPixelFormats.FLOAT16_RGB:
                        return AEPixelFormats.FLOAT32_RGB;

                    case AEPixelFormats.FLOAT16_RGBA:
                        return AEPixelFormats.FLOAT32_RGBA;

                    default:
                        break;
                }
                break;

            default:
                break;
        }

        return eFmt;
    }
    exports.getFormatForBitDepths = getFormatForBitDepths;

    /** Pack a colour value to memory
    @param colour	The colour
    @param pf		Pixelformat in which to write the colour
    @param dest		Destination memory location
    */
    /** inline */ function packColour(cColour, ePf, pDest) {
        exports.packColourFloat(cColour.r, cColour.g, cColour.b, cColour.a, ePf, pDest);
    }

    /** Pack a colour value to memory
    @param r,g,b,a	The four colour components, range 0x00 to 0xFF
    @param pf		Pixelformat in which to write the colour
    @param dest		Destination memory location
    */
    function packColourUint(r, g, b, a, ePf, pDest) {
        // if (arguments.length < 4) {
        // 	var cColour: AIColor = arguments[0];
        // 	packColour(cColour.r, cColour.g, cColour.b, cColour.a, ePf, pDest);
        // 	return;
        // }
        /** @const */ var des = exports.getDescriptionFor(ePf);
        if (des.flags & AEPixelFormatFlags.NATIVEENDIAN) {
            // Shortcut for integer formats packing
            var value = ((bf.fixedToFixed(r, 8, des.rbits) << des.rshift) & des.rmask) | ((bf.fixedToFixed(g, 8, des.gbits) << des.gshift) & des.gmask) | ((bf.fixedToFixed(b, 8, des.bbits) << des.bshift) & des.bmask) | ((bf.fixedToFixed(a, 8, des.abits) << des.ashift) & des.amask);

            // And write to memory
            bf.intWrite(pDest, des.elemBytes, value);
        } else {
            // Convert to float
            exports.packColourFloat(r / 255.0, g / 255.0, b / 255.0, a / 255.0, ePf, pDest);
        }
    }
    exports.packColourUint = packColourUint;

    /** Pack a colour value to memory
    @param r,g,b,a	The four colour components, range 0.0f to 1.0f
    (an exception to this case exists for floating point pixel
    formats, which don't clamp to 0.0f..1.0f)
    @param pf		Pixelformat in which to write the colour
    @param dest		Destination memory location
    */
    function packColourFloat(r, g, b, a, ePf, pDest) {
        // Catch-it-all here
        /** @const */ var des = exports.getDescriptionFor(ePf);
        if (des.flags & AEPixelFormatFlags.NATIVEENDIAN) {
            // Do the packing
            //std::cerr << dest << " " << r << " " << g <<  " " << b << " " << a << std::endl;
            /** @const */ var value = ((bf.floatToFixed(r, des.rbits) << des.rshift) & des.rmask) | ((bf.floatToFixed(g, des.gbits) << des.gshift) & des.gmask) | ((bf.floatToFixed(b, des.bbits) << des.bshift) & des.bmask) | ((bf.floatToFixed(a, des.abits) << des.ashift) & des.amask);

            // And write to memory
            bf.intWrite(pDest, des.elemBytes, value);
        } else {
            switch (ePf) {
                case AEPixelFormats.FLOAT32_R:
                    dynamic_cast_f32_ptr(pDest, 1)[0] = r;
                    break;
                case AEPixelFormats.FLOAT32_GR:
                    dynamic_cast_f32_ptr(pDest, 1)[0] = g;
                    dynamic_cast_f32_ptr(pDest, 2)[1] = r;
                    break;
                case AEPixelFormats.FLOAT32_RGB:
                    dynamic_cast_f32_ptr(pDest, 1)[0] = r;
                    dynamic_cast_f32_ptr(pDest, 2)[1] = g;
                    dynamic_cast_f32_ptr(pDest, 3)[2] = b;
                    break;
                case AEPixelFormats.FLOAT32_RGBA:
                    dynamic_cast_f32_ptr(pDest, 1)[0] = r;
                    dynamic_cast_f32_ptr(pDest, 2)[1] = g;
                    dynamic_cast_f32_ptr(pDest, 3)[2] = b;
                    dynamic_cast_f32_ptr(pDest, 4)[3] = a;
                    break;
                case AEPixelFormats.FLOAT16_R:
                    dynamic_cast_u16_ptr(pDest, 1)[0] = bf.floatToHalf(r);
                    break;
                case AEPixelFormats.FLOAT16_GR:
                    dynamic_cast_u16_ptr(pDest, 1)[0] = bf.floatToHalf(g);
                    dynamic_cast_u16_ptr(pDest, 2)[1] = bf.floatToHalf(r);
                    break;
                case AEPixelFormats.FLOAT16_RGB:
                    dynamic_cast_u16_ptr(pDest, 1)[0] = bf.floatToHalf(r);
                    dynamic_cast_u16_ptr(pDest, 2)[1] = bf.floatToHalf(g);
                    dynamic_cast_u16_ptr(pDest, 3)[2] = bf.floatToHalf(b);
                    break;
                case AEPixelFormats.FLOAT16_RGBA:
                    dynamic_cast_u16_ptr(pDest, 1)[0] = bf.floatToHalf(r);
                    dynamic_cast_u16_ptr(pDest, 2)[1] = bf.floatToHalf(g);
                    dynamic_cast_u16_ptr(pDest, 3)[2] = bf.floatToHalf(b);
                    dynamic_cast_u16_ptr(pDest, 4)[3] = bf.floatToHalf(a);
                    break;
                case AEPixelFormats.SHORT_RGB:
                    dynamic_cast_u16_ptr(pDest, 1)[0] = bf.floatToFixed(r, 16);
                    dynamic_cast_u16_ptr(pDest, 2)[1] = bf.floatToFixed(g, 16);
                    dynamic_cast_u16_ptr(pDest, 3)[2] = bf.floatToFixed(b, 16);
                    break;
                case AEPixelFormats.SHORT_RGBA:
                    dynamic_cast_u16_ptr(pDest, 1)[0] = bf.floatToFixed(r, 16);
                    dynamic_cast_u16_ptr(pDest, 2)[1] = bf.floatToFixed(g, 16);
                    dynamic_cast_u16_ptr(pDest, 3)[2] = bf.floatToFixed(b, 16);
                    dynamic_cast_u16_ptr(pDest, 4)[3] = bf.floatToFixed(a, 16);
                    break;
                case AEPixelFormats.BYTE_LA:
                    pDest[0] = bf.floatToFixed(r, 8);
                    pDest[1] = bf.floatToFixed(a, 8);
                    break;
                default:
                    // Not yet supported
                    logger.error("pack to " + exports.getFormatName(ePf) + " not implemented", "PixelUtil::packColour");
                    break;
            }
        }
    }
    exports.packColourFloat = packColourFloat;

    /** Unpack a colour value from memory
    @param colour	The colour is returned here
    @param pf		Pixelformat in which to read the colour
    @param src		Source memory location
    */
    function unpackColour(cColour, ePf, pSrc) {
        exports.unpackColourFloat(cColour, ePf, pSrc);
    }
    exports.unpackColour = unpackColour;

    /** Unpack a colour value from memory
    @param r,g,b,a	The colour is returned here (as byte)
    @param pf		Pixelformat in which to read the colour
    @param src		Source memory location
    @remarks 	This function returns the colour components in 8 bit precision,
    this will lose precision when coming from PF_A2R10G10B10 or floating
    point formats.
    */
    function unpackColourUint(rgba, ePf, pSrc) {
        /** @const */ var des = exports.getDescriptionFor(ePf);
        var r = 0, g = 0, b = 0, a = 0;

        if (des.flags & AEPixelFormatFlags.NATIVEENDIAN) {
            // Shortcut for integer formats unpacking
            /** @const */ var value = bf.intRead(pSrc, des.elemBytes);
            if (des.flags & AEPixelFormatFlags.LUMINANCE) {
                // Luminance format -- only rbits used
                r = g = b = bf.fixedToFixed((value & des.rmask) >> des.rshift, des.rbits, 8);
            } else {
                r = bf.fixedToFixed((value & des.rmask) >> des.rshift, des.rbits, 8);
                g = bf.fixedToFixed((value & des.gmask) >> des.gshift, des.gbits, 8);
                b = bf.fixedToFixed((value & des.bmask) >> des.bshift, des.bbits, 8);
            }
            if (des.flags & AEPixelFormatFlags.HASALPHA) {
                a = bf.fixedToFixed((value & des.amask) >> des.ashift, des.abits, 8);
            } else {
                a = 255;
            }
        } else {
            // Do the operation with the more generic floating point
            var pRGBA = _pColorValue;
            exports.unpackColourFloat(pRGBA, ePf, pSrc);

            r = bf.floatToFixed(pRGBA.r, 8);
            g = bf.floatToFixed(pRGBA.g, 8);
            b = bf.floatToFixed(pRGBA.b, 8);
            a = bf.floatToFixed(pRGBA.a, 8);
        }

        rgba[0] = r;
        rgba[1] = g;
        rgba[2] = b;
        rgba[3] = a;
    }
    exports.unpackColourUint = unpackColourUint;

    /** Unpack a colour value from memory
    @param r,g,b,a	The colour is returned here (as float)
    @param pf		Pixelformat in which to read the colour
    @param src		Source memory location
    */
    function unpackColourFloat(rgba, ePf, pSrc) {
        /** @const */ var des = exports.getDescriptionFor(ePf);
        var r = 0., g = 0., b = 0., a = 0.;

        if (des.flags & AEPixelFormatFlags.NATIVEENDIAN) {
            // Shortcut for integer formats unpacking
            /** @const */ var value = bf.intRead(pSrc, des.elemBytes);

            if (des.flags & AEPixelFormatFlags.LUMINANCE) {
                // Luminance format -- only rbits used
                r = g = b = bf.fixedToFloat((value & des.rmask) >>> des.rshift, des.rbits);
            } else {
                r = bf.fixedToFloat((value & des.rmask) >>> des.rshift, des.rbits);
                g = bf.fixedToFloat((value & des.gmask) >>> des.gshift, des.gbits);
                b = bf.fixedToFloat((value & des.bmask) >>> des.bshift, des.bbits);
            }

            if (des.flags & AEPixelFormatFlags.HASALPHA) {
                a = bf.fixedToFloat((value & des.amask) >>> des.ashift, des.abits);
            } else {
                a = 1.0;
            }
        } else {
            switch (ePf) {
                case AEPixelFormats.FLOAT32_DEPTH:
                case AEPixelFormats.FLOAT32_R:
                    r = g = b = dynamic_cast_f32_ptr(pSrc, 1)[0];
                    a = 1.0;
                    break;
                case AEPixelFormats.FLOAT32_GR:
                    g = dynamic_cast_f32_ptr(pSrc, 1)[0];
                    r = b = dynamic_cast_f32_ptr(pSrc, 2)[1];
                    a = 1.0;
                    break;
                case AEPixelFormats.FLOAT32_RGB:
                    r = dynamic_cast_f32_ptr(pSrc, 1)[0];
                    g = dynamic_cast_f32_ptr(pSrc, 2)[1];
                    b = dynamic_cast_f32_ptr(pSrc, 3)[2];
                    a = 1.0;
                    break;
                case AEPixelFormats.FLOAT32_RGBA:
                    r = dynamic_cast_f32_ptr(pSrc, 1)[0];
                    g = dynamic_cast_f32_ptr(pSrc, 2)[1];
                    b = dynamic_cast_f32_ptr(pSrc, 3)[2];
                    a = dynamic_cast_f32_ptr(pSrc, 4)[3];
                    break;
                case AEPixelFormats.FLOAT16_R:
                    r = g = b = bf.halfToFloat((dynamic_cast_u16_ptr(pSrc, 1))[0]);
                    a = 1.0;
                    break;
                case AEPixelFormats.FLOAT16_GR:
                    g = bf.halfToFloat((dynamic_cast_u16_ptr(pSrc, 1))[0]);
                    r = b = bf.halfToFloat((dynamic_cast_u16_ptr(pSrc, 2))[1]);
                    a = 1.0;
                    break;
                case AEPixelFormats.FLOAT16_RGB:
                    r = bf.halfToFloat((dynamic_cast_u16_ptr(pSrc, 1))[0]);
                    g = bf.halfToFloat((dynamic_cast_u16_ptr(pSrc, 1))[1]);
                    b = bf.halfToFloat((dynamic_cast_u16_ptr(pSrc, 2))[2]);
                    a = 1.0;
                    break;
                case AEPixelFormats.FLOAT16_RGBA:
                    r = bf.halfToFloat((dynamic_cast_u16_ptr(pSrc, 1))[0]);
                    g = bf.halfToFloat((dynamic_cast_u16_ptr(pSrc, 2))[1]);
                    b = bf.halfToFloat((dynamic_cast_u16_ptr(pSrc, 3))[2]);
                    a = bf.halfToFloat((dynamic_cast_u16_ptr(pSrc, 4))[3]);
                    break;
                case AEPixelFormats.SHORT_RGB:
                    r = bf.fixedToFloat((dynamic_cast_u16_ptr(pSrc, 1))[0], 16);
                    g = bf.fixedToFloat((dynamic_cast_u16_ptr(pSrc, 2))[1], 16);
                    b = bf.fixedToFloat((dynamic_cast_u16_ptr(pSrc, 3))[2], 16);
                    a = 1.0;
                    break;
                case AEPixelFormats.SHORT_RGBA:
                    r = bf.fixedToFloat((dynamic_cast_u16_ptr(pSrc, 1))[0], 16);
                    g = bf.fixedToFloat((dynamic_cast_u16_ptr(pSrc, 2))[1], 16);
                    b = bf.fixedToFloat((dynamic_cast_u16_ptr(pSrc, 3))[2], 16);
                    a = bf.fixedToFloat((dynamic_cast_u16_ptr(pSrc, 4))[3], 16);
                    break;
                case AEPixelFormats.BYTE_LA:
                    r = g = b = bf.fixedToFloat((pSrc)[0], 8);
                    a = bf.fixedToFloat((pSrc)[1], 8);
                    break;
                default:
                    // Not yet supported
                    logger.error("unpack from " + exports.getFormatName(ePf) + " not implemented", "PixelUtil::unpackColour");
                    break;
            }
        }

        rgba.r = r;
        rgba.g = g;
        rgba.b = b;
        rgba.a = a;
    }
    exports.unpackColourFloat = unpackColourFloat;

    function bulkPixelConversion(pSrc, eSrcFormat, pDest, eDstFormat, iCount) {
        var src = null, dst = null;

        if (arguments.length > 2) {
            src = new PixelBox(iCount, 1, 1, eSrcFormat, pSrc);
            dst = new PixelBox(iCount, 1, 1, eDstFormat, pDest);
        } else {
            src = arguments[0];
            dst = arguments[1];
        }

        if (src.width !== dst.width || src.height !== dst.height || src.depth !== dst.depth) {
            logger.critical("Size dest and src pictures is different");
            return;
        }

        if (exports.isCompressed(src.format) || exports.isCompressed(dst.format)) {
            if (src.format == dst.format) {
                //_memcpy(dst.data.buffer, src.data.buffer, src.getConsecutiveSize());
                dst.data.set(src.data.subarray(0, src.getConsecutiveSize()));
                return;
            } else {
                logger.error("This method can not be used to compress or decompress images", "PixelUtil::bulkPixelConversion");
            }
        }

        if (src.format == dst.format) {
            if (src.isConsecutive() && dst.isConsecutive()) {
                //_memcpy(dst.data.buffer, src.data.buffer, src.getConsecutiveSize());
                dst.data.set(src.data.subarray(0, src.getConsecutiveSize()));
                return;
            }

            var srcPixelSize = exports.getNumElemBytes(src.format);
            var dstPixelSize = exports.getNumElemBytes(dst.format);

            var srcptr = src.data.subarray((src.left + src.top * src.rowPitch + src.front * src.slicePitch) * srcPixelSize);
            var dstptr = dst.data.subarray((dst.left + dst.top * dst.rowPitch + dst.front * dst.slicePitch) * dstPixelSize);

            // Calculate pitches+skips in bytes
            var srcRowPitchBytes = src.rowPitch * srcPixelSize;

            //var size_t srcRowSkipBytes = src.getRowSkip()*srcPixelSize;
            var srcSliceSkipBytes = src.getSliceSkip() * srcPixelSize;

            var dstRowPitchBytes = dst.rowPitch * dstPixelSize;

            //var size_t dstRowSkipBytes = dst.getRowSkip()*dstPixelSize;
            var dstSliceSkipBytes = dst.getSliceSkip() * dstPixelSize;

            // Otherwise, copy per row
            /** @const */ var rowSize = src.width * srcPixelSize;

            for (var z = src.front; z < src.back; z++) {
                for (var y = src.top; y < src.bottom; y++) {
                    //_memcpy(dstptr.buffer, srcptr.buffer, rowSize);
                    dstptr.set(srcptr.subarray(0, rowSize));

                    srcptr = srcptr.subarray(srcRowPitchBytes);
                    dstptr = dstptr.subarray(dstRowPitchBytes);
                }

                srcptr = srcptr.subarray(srcSliceSkipBytes);
                dstptr = dstptr.subarray(dstSliceSkipBytes);
            }

            return;
        }

        if (dst.format == AEPixelFormats.X8R8G8B8 || dst.format == AEPixelFormats.X8B8G8R8) {
            // Do the same conversion, with AEPixelFormats.A8R8G8B8, which has a lot of
            // optimized conversions
            var tempdst = dst;
            tempdst.format = (dst.format == AEPixelFormats.X8R8G8B8) ? AEPixelFormats.A8R8G8B8 : AEPixelFormats.A8B8G8R8;
            exports.bulkPixelConversion(src, tempdst);
            return;
        }

        if ((src.format == AEPixelFormats.X8R8G8B8 || src.format == AEPixelFormats.X8B8G8R8) && !exports.hasAlpha(dst.format)) {
            // Do the same conversion, with AEPixelFormats.A8R8G8B8, which has a lot of
            // optimized conversions
            var tempsrc = src;
            tempsrc.format = src.format == AEPixelFormats.X8R8G8B8 ? AEPixelFormats.A8R8G8B8 : AEPixelFormats.A8B8G8R8;
            exports.bulkPixelConversion(tempsrc, dst);
            return;
        }

        var srcPixelSize = exports.getNumElemBytes(src.format);
        var dstPixelSize = exports.getNumElemBytes(dst.format);

        var srcptr = src.data.subarray((src.left + src.top * src.rowPitch + src.front * src.slicePitch) * srcPixelSize);
        var dstptr = dst.data.subarray((dst.left + dst.top * dst.rowPitch + dst.front * dst.slicePitch) * dstPixelSize);

        // Old way, not taking into account box dimensions
        //uint8 *srcptr = static_cast<uint8*>(src.data), *dstptr = static_cast<uint8*>(dst.data);
        // Calculate pitches+skips in bytes
        var srcRowSkipBytes = src.getRowSkip() * srcPixelSize;
        var srcSliceSkipBytes = src.getSliceSkip() * srcPixelSize;
        var dstRowSkipBytes = dst.getRowSkip() * dstPixelSize;
        var dstSliceSkipBytes = dst.getSliceSkip() * dstPixelSize;

        // The brute force fallback
        // var r: float = 0, g: float = 0, b: float = 0, a: float = 1;
        var rgba = _pColorValue;
        for (var z = src.front; z < src.back; z++) {
            for (var y = src.top; y < src.bottom; y++) {
                for (var x = src.left; x < src.right; x++) {
                    exports.unpackColourFloat(rgba, src.format, srcptr);
                    exports.packColourFloat(rgba.r, rgba.g, rgba.b, rgba.a, dst.format, dstptr);
                    srcptr = srcptr.subarray(srcPixelSize);
                    dstptr = dstptr.subarray(dstPixelSize);
                }
                srcptr = srcptr.subarray(srcRowSkipBytes);
                dstptr = dstptr.subarray(dstRowSkipBytes);
            }
            srcptr = srcptr.subarray(srcSliceSkipBytes);
            dstptr = dstptr.subarray(dstSliceSkipBytes);
        }
    }
    exports.bulkPixelConversion = bulkPixelConversion;

    function calculateSizeForImage(nMipLevels, nFaces, iWidth, iHeight, iDepth, eFormat) {
        var iSize = 0;
        var mip = 0;

        for (mip = 0; mip < nMipLevels; ++mip) {
            iSize += exports.getMemorySize(iWidth, iHeight, iDepth, eFormat) * nFaces;
            if (iWidth !== 1)
                iWidth /= 2;
            if (iHeight !== 1)
                iHeight /= 2;
            if (iDepth !== 1)
                iDepth /= 2;
        }

        return iSize;
    }
    exports.calculateSizeForImage = calculateSizeForImage;
});
//# sourceMappingURL=pixelUtil.js.map
