...
import ComponentPassInputBlend = require("fx/ComponentPassInputBlend");
...
class ComponentBlend implements AIAFXComponentBlend {
    //UNIQUE();
    ...
    private _pComponentHashMap: AIMap<boolean> = null;

    private _pAddedComponentInfoList: AIAFXComponentInfo[] = null;
    ...
    containComponent(pComponent: AIAFXComponent, iShift: int, iPass: uint): boolean {
        ...
        if (iCorrectShift !== ANY_SHIFT && iCorrectPass !== ANY_PASS) {
            ...
            for (var i: uint = 0; i < this._pAddedComponentInfoList.length; i++) {
                var pInfo: AIAFXComponentInfo = this._pAddedComponentInfoList[i];
    ...
    addComponent(pComponent: AIAFXComponent, iShift: int, iPass: int): void {
        ...
        var pInfo: AIAFXComponentInfo = <AIAFXComponentInfo>{
    ...
    finalizeBlend(): boolean {
        ...
        for (var i: uint = 0; i < this._pAddedComponentInfoList.length; i++) {
            ...
            var pComponentTechnique: AIAFXTechniqueInstruction = pInfo.component.getTechnique();
            var iShift: int = pInfo.shift - this._iShiftMin;
            var iPass: int = pInfo.pass;

            var pPass: AIAFXPassInstruction = pComponentTechnique.getPass(iPass);
    ...
    clone(): AIAFXComponentBlend {
        var pClone: AIAFXComponentBlend = new ComponentBlend(this._pComposer);
    ...
    _setDataForClone(pComponentInfoList: AIAFXComponentInfo[],
        pComponentHashMap: AIMap<boolean>,
