    ...
    (function (parser) {
        ...
        var Parser = (function () {
            ...
            Parser.prototype.nextState_LR0 = function (pState, sSymbol) {
                ...
                for (i = 0; i < pItemList.length; i++) {
                    if (sSymbol === pItemList[i].mark()) {
                        pNewState.push(new parser.Item(pItemList[i].rule, pItemList[i].position + 1));
                    }
                }
            ...
            Parser.prototype.nextState_LR = function (pState, sSymbol) {
                ...
                for (i = 0; i < pItemList.length; i++) {
                    if (sSymbol === pItemList[i].mark()) {
                        pNewState.push(new parser.Item(pItemList[i].rule, pItemList[i].position + 1, pItemList[i].expectedSymbols));
                    }
                }
            ...
            Parser.prototype.addReducing = function (pState) {
                ...
                for (i = 0; i < pItemList.length; i++) {
                    if (pItemList[i].mark() === parser.END_POSITION) {
                        if (pItemList[i].rule.left === parser.START_SYMBOL) {
                            this.pushInSyntaxTable(pState.index, parser.END_SYMBOL, this._pSuccessOperation);
                        } else {
                            var pExpected = pItemList[i].expectedSymbols;
