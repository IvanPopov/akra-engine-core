...
import ImgCodec = require("pixelUtil/ImgCodec");
...
class DDSCodec extends ImgCodec implements AIDDSCodec {
    private _sType: string = "dds";
    private static _pInstance: AIDDSCodec = null;
    ...
    decode(pData: Uint8Array, pImgData: AIImgData): Uint8Array {
        var iOffset: uint = 0;
        ...
        if (pHeader.dwCaps2 & DDSCAPS2_CUBEMAP) {
            pImgData.flags |= AEImageFlags.CUBEMAP;
            nFace = 0;
            if (pHeader.dwCaps2 & DDSCAPS2_CUBEMAP_POSITIVEX) {
                nFace++;
                pImgData.cubeFlags |= AEImageCubeFlags.POSITIVE_X;
            }
            if (pHeader.dwCaps2 & DDSCAPS2_CUBEMAP_NEGATIVEX) {
                nFace++;
                pImgData.cubeFlags |= AEImageCubeFlags.NEGATIVE_X;
            }
            if (pHeader.dwCaps2 & DDSCAPS2_CUBEMAP_POSITIVEY) {
                nFace++;
                pImgData.cubeFlags |= AEImageCubeFlags.POSITIVE_Y;
            }
            if (pHeader.dwCaps2 & DDSCAPS2_CUBEMAP_NEGATIVEY) {
                nFace++;
                pImgData.cubeFlags |= AEImageCubeFlags.NEGATIVE_Y;
            }
            if (pHeader.dwCaps2 & DDSCAPS2_CUBEMAP_POSITIVEZ) {
                nFace++;
                pImgData.cubeFlags |= AEImageCubeFlags.POSITIVE_Z;
            }
            if (pHeader.dwCaps2 & DDSCAPS2_CUBEMAP_NEGATIVEZ) {
                nFace++;
                pImgData.cubeFlags |= AEImageCubeFlags.NEGATIVE_Z;
            }
        ...
        if (pHeader.ddspf.dwFlags & DDPF_FOURCC) {
            if (pHeader.ddspf.dwFourCC == D3DFMT_DXT1) {
                ...
                eSourceFormat = AEPixelFormats.FLOAT16_R;
            }
            else if (pHeader.ddspf.dwFourCC == D3DFMT_G16R16F) {
                eSourceFormat = AEPixelFormats.FLOAT16_GR;
            }
            else if (pHeader.ddspf.dwFourCC == D3DFMT_A16B16G16R16F) {
                eSourceFormat = AEPixelFormats.FLOAT16_RGBA;
            }
            else if (pHeader.ddspf.dwFourCC == D3DFMT_R32F) {
                eSourceFormat = AEPixelFormats.FLOAT32_R;
            }
            else if (pHeader.ddspf.dwFourCC == D3DFMT_G32R32F) {
                eSourceFormat = AEPixelFormats.FLOAT32_GR;
            }
            else if (pHeader.ddspf.dwFourCC == D3DFMT_A32B32G32R32F) {
                eSourceFormat = AEPixelFormats.FLOAT32_RGBA;
            }
            else {
            ...
            var ePF: AEPixelFormats;
            for (ePF = AEPixelFormats.UNKNOWN + 1; ePF < AEPixelFormats.TOTAL; ePF++) {
