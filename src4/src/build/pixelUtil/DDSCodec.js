        ...
        var DDSCodec = (function (_super) {
            ...
            DDSCodec.startup = function () {
                if (!akra.isDefAndNotNull(this._pInstance)) {
                    ...
                    akra.pixelUtil.Codec.registerCodec(this._pInstance);
            ...
            DDSCodec.shutdown = function () {
                if (akra.isDef(this._pInstance)) {
                    akra.pixelUtil.Codec.unRegisterCodec(this._pInstance);
            ...
            DDSCodec.prototype.decode = function (pData, pImgData) {
                ...
                if (pHeader.ddspf.dwFlags & DDPF_FOURCC) {
                    ...
                    for (ePF = 0 /* UNKNOWN */ + 1; ePF < 48 /* TOTAL */; ePF++) {
                        if ((!!(pHeader.ddspf.dwFlags & DDPF_LUMINANCE)) != akra.pixelUtil.isLuminance(ePF)) {
                            continue;
                        }
                        if ((!!(pHeader.ddspf.dwFlags & DDPF_ALPHAPIXELS)) != akra.pixelUtil.hasAlpha(ePF)) {
                            continue;
                        }
                ...
                for (var i = 0; i < nFace; i++) {
                    ...
                    for (var iMip = 0; iMip <= pImgData.getNumMipMaps(); iMip++) {
                        if (akra.pixelUtil.isCompressed(pImgData.getFormat())) {
                            var iDXTSize = akra.pixelUtil.getMemorySize(iWidth, iHeight, iDepth, pImgData.getFormat());
                            ...
                            var iDstPitch = iWidth * akra.pixelUtil.getNumElemBytes(pImgData.getFormat());
                            var iSrcPitch = 0;
