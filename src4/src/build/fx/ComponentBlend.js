    ...
    (function (fx) {
        var ComponentBlend = (function () {
            ...
            ComponentBlend.prototype.containComponent = function (pComponent, iShift, iPass) {
                ...
                if (iShift === akra.fx.DEFAULT_SHIFT) {
                    if (pComponent.isPostEffect()) {
                        iCorrectShift = akra.fx.ANY_PASS;
                    } else {
                        iCorrectShift = 0;
                    }
                }

                if (iCorrectShift !== akra.fx.ANY_SHIFT && iCorrectPass !== akra.fx.ANY_PASS) {
                    ...
                    for (var i = 0; i < this._pAddedComponentInfoList.length; i++) {
                        ...
                        if (pInfo.component === pComponent) {
                            if (iCorrectShift === akra.fx.ANY_SHIFT && iCorrectPass === akra.fx.ANY_PASS) {
                                return true;
                            } else if (iCorrectShift === akra.fx.ANY_SHIFT && pInfo.pass === iCorrectPass) {
                                return true;
                            } else if (iCorrectPass === akra.fx.ANY_PASS && pInfo.shift === iCorrectShift) {
                                return true;
                            }
                        }
                    }
            ...
            ComponentBlend.prototype.findAddedComponentInfo = function (pComponent, iShift, iPass) {
                ...
                if (iCorrectShift !== akra.fx.ANY_SHIFT && iCorrectPass !== akra.fx.ANY_PASS && !this.containComponentHash(pComponent.getHash(iCorrectShift, iCorrectPass))) {
                    return null;
                } else {
                    for (var i = 0; i < this._pAddedComponentInfoList.length; i++) {
                        var pInfo = this._pAddedComponentInfoList[i];
            ...
            ComponentBlend.prototype.finalizeBlend = function () {
                ...
                for (var i = 0; i < this._pAddedComponentInfoList.length; i++) {
                    ...
                    if (!akra.isDef(this._pPassesDList[iShift])) {
                        ...
                        this._pComponentInputVarBlend[iShift] = new akra.fx.ComponentPassInputBlend();
                    }
