...
module akra.ui.graph {
	export class ConnectionArea extends Panel implements IUIGraphConnectionArea {
		...
		protected _pConnectors: IUIGraphConnector[] = new Array<Connector>();
		...
		getNode(): IUIGraphNode { return <IUIGraphNode>this.getParent(); }
		getGraph(): IUIGraph { return this.getNode().getGraph(); }

		setMaxInConnections(n: uint) {
			this._iInConnectionLimit = n;
		}

		setMaxOutConnections(n: uint) {
			this._iOutConnectionLimit = n;
		}

		setMaxConnections(n: uint) {
		...
		findRoute(pNode: IUIGraphNode): IUIGraphRoute {
			for (var i: int = 0; i < this._pConnectors.length; ++i) {
				var pRoute: IUIGraphRoute = this._pConnectors[i].getRoute();
		...
		hasConnections(): boolean {
			return !(this.getConnectors().length == 0 || isNull(this.getConnectors()[0]));
		}
		...
		sendEvent(e: IUIGraphEvent): void {
			for (var i = 0; i < this._pConnectors.length; ++i) {
				if (this._pConnectors[i].getDirection() === EUIGraphDirections.OUT) {
		...
		_onNodeMouseover(pNode: IUIGraphNode, e: IUIEvent): void {
			...
			if ((!this.isSupportsIncoming() && this.getGraph().isReadyForConnect()) ||
				(!this.isSupportsOutgoing() && !this.getGraph().isReadyForConnect())) {
				return;
			}
