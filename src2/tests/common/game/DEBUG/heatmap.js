...
/*---------------------------------------------
 * assembled at: Sat Nov 02 2013 01:36:34 GMT+0400 (Московское время (зима))
 * directory: tests/common/game/DEBUG/
 * file: tests/common/game/heatmap.ts
 * name: heatmap
 *--------------------------------------------*/
...
(function (akra) {
    ...
    var pOptions = {
        ...
        deps: {
            files: [
                {
                    path: "http://akra/akra-engine-core/src2/tests/common/game/DEBUG/bonner.jpg",
                    name: "akra_logo"
                }, 
                {
                    path: "http://akra/akra-engine-core/src2/tests/common/game/DEBUG/girl.jpg",
                    name: "girl"
                }, 
                {
    ...
    function main(pEngine) {
        setup(pCanvas);
        var pGirlImage = pRmgr.loadImage("girl");
        var pLogoImage = pRmgr.loadImage("akra_logo");
        var pLogo = pRmgr.createTexture("akra_logo_texture");
        var pGirl = pRmgr.createTexture("girl_texture");
        pLogo.loadImage(pLogoImage);
        pGirl.loadImage(pGirlImage);
        pViewport = pCanvas.addViewport(new akra.render.TextureViewport(pLogo));
        ...
        pGUI.add({
            ...
            if (use) {
                (pViewport).effect.addComponent("akra.custom.heatmap", 1, 0);
            } else {
                (pViewport).effect.delComponent("akra.custom.heatmap", 1, 0);
            }
        });
        (pGUI.add(pHSV, "hue")).min(0.).max(10.).step(0.01);
        (pGUI.add(pHSV, "saturation")).min(0.).max(10.).step(0.01);
        (pGUI.add(pHSV, "value")).min(0.).max(10.).step(0.01);
